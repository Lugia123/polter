...
**读输出,不要只读退出码**:其中几道是棘轮——数字比基线*低*也会失败,
消息会告诉你把基线改成多少。`no-local-identifiers.py` 还带一个正控制。

许多闸打一行 `probe self-test: OK`,那是它在说自己的读取器还认得被测的形状;
**打了这一行的闸,它不出现就说明检查器本身没工作**——那时下面的判定一个字都不算数。
⚠️ **但不是每道闸都打这一行,所以「没有这一行」本身不是坏消息。**
原话写的是「每一道闸都带自我检查,那一行不出现就说明检查器没工作」,
而今天七十多道里有二十几道根本不打——照那句话读,会把一批正常的闸判成坏的。
**要知道某一道有没有自检,去读那一道**;这里不写个数,理由和上面 glob 那条一样。

⚠️ **自检通过不等于检查器还看得见被测对象。** `the-frames-permit-comes-back.py`
栽过:上游把 `swap_chain` 改成 optional、写法多了一个 `.?`,它的正则就再也匹配不上,
于是它对着一棵**正确**的树喊「根本没有归还许可」。自检当时是 OK 的,
**因为探针和被测代码是同一种写法**——探针跟着被测对象一起写,就测不出写法变了。
所以探针要**连不再使用的和新出现的拼法一起放**,别只放今天这一种。
