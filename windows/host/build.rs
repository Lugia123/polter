...
/// List every `src/*.rs` file for the catalogue test to read.
///
/// **The point is the default.** The first version of that test carried a
/// hand-written list of files and a comment asking people to add theirs. A
/// file nobody remembered to add was simply not checked -- and *not checked*
/// looked exactly like *nothing wrong*, which is the same shape of silence
/// the test was written to end. One file was already missed that way within
/// the hour.
///
/// So the list is generated here and it is everything. A file that needs to
/// stay out is named in the test, next to a sentence saying why, where the
/// next reader sees it.
fn i18n_sources() {
    use std::fmt::Write as _;

    // A new file in this directory has to regenerate the list, and a new file
    // does not change any file already in it -- so the directory itself is
    // what has to be watched, not just its contents.
    println!("cargo:rerun-if-changed=src");

    let mut names: Vec<String> = std::fs::read_dir("src")
        .expect("windows/host/src must be readable to build")
        .filter_map(|e| e.ok())
    ...
    let dir = std::fs::canonicalize("src").expect("windows/host/src must resolve");
    let mut out = String::new();
    out.push_str("// Generated by build.rs: every `.rs` file in windows/host/src,\n");
    out.push_str("// with no filtering. See `i18n_sources` in build.rs for why.\n");
    out.push_str("const SOURCES: &[(&str, &str)] = &[\n");
    for n in &names {
        // Forward slashes even on Windows: this string goes into an
        // `include_str!`, where a backslash is an escape.
        let path = dir.join(n).to_string_lossy().replace('\\', "/");
        writeln!(out, "    ({n:?}, include_str!({path:?})),").unwrap();
        println!("cargo:rerun-if-changed=src/{n}");
    }
    out.push_str("];\n");

    let dest = std::path::PathBuf::from(std::env::var("OUT_DIR").unwrap())
        .join("i18n-sources.rs");
    std::fs::write(&dest, out).expect("could not write the i18n source list");
}
