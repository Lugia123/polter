    ...
    unsafe {
        ...
        for r in rows {
            ...
            if let Some(children) = r.sub {
                ...
                // **Translated here, not in the table.** The tables are
                // `const` and a `const` cannot call a function, so the label
                // in them is the English msgid marked with `n_` and the
                // lookup happens at the moment it is put on screen.
...
mod tests {
    ...
    fn the_greyed_rows_are_the_three_that_are_not_built() {
        ...
        let mut want = vec!["Language…", "Check for Updates…", "Polter Help"];
        want.sort_unstable();
