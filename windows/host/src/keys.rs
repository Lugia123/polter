...
/// **How to pick a chord for this table, learned the hard way.** Do not
/// probe empirically -- press a candidate and see whether anything happens.
/// Two things make that worse than it sounds. A chord the core already binds
/// never reaches this table at all, so "nothing happened" and "it worked and
/// did nothing" look identical from the keyboard. And this table is not the
/// only thing a chord can reach: probing on a real machine, `ctrl+shift+-`
/// turned out not to be in the core's table either, and pressing it opened
/// that host's own local chat and task panel -- reading real Poltergeist
/// state out of the user's profile, with a `#32770` confirm-close dialog on
/// top. No send was seen in that run's log, but whether a send would be
/// logged there was never checked, so that stays unverified rather than
/// being written down as "nothing was sent".
///
/// Read `src/config/Config.zig` instead: every row with
/// `.{ .ctrl = true, .shift = true }`. Two readings of that file disagreed
/// once -- a text search for `ctrl+shift+<letter>` matched mostly prose in
/// comments and missed two letters bound through a `mods` variable
/// (`c` and `v`, copy and paste on anything but macOS) -- so read the
/// struct literals, and when two lists disagree treat every difference as
/// taken.
