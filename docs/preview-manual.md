...
`src/os/i18n.zig` 的 `locales_map` 和 `staticLocale` 里一共有 **5 处编译错误**，
分属 3 个互不相干的成因，而全量测试一直是绿的。原因是 **Zig 惰性分析容器级
声明**：没有任何地方引用它们，它们就不会被求值，坏成什么样都不会有人知道。
修复见 `src/os/i18n.zig` 底部那条测试。

**并不是所有错误都藏得住。**下表每一格都是在本仓真编一次量出来的，不是推的
（做法：往一个文件里放一个坏声明，跑 `zig build test`，看退出码）：

| 引用方式 | `const` 初始值里的错误 | `fn` 函数体里的错误 |
| --- | --- | --- |
| 没有任何引用 | **不报**（惰性，从没分析） | **不报** |
| `_ = x;` | 报 | **不报**（只取了名字） |
| `_ = &x;` | 报 | 报 |
| `std.testing.refAllDecls(@This())` | 报 | 报 |

⚠️ **一个容易把实验做反的陷阱**：如果你用 `_ = 某个不存在的标识符` 来造这个
「坏声明」，**每一格都会红**——未声明标识符是名字解析错误，按文件早求值，与
引不引用无关。要复现这一族，坏法必须是**类型错误**（例如把模块当数组用），
它才随声明惰性求值。

**所以这道闸已经在语言里**：`std.testing.refAllDecls(@This())`。写了它的文件，
容器级声明一定被分析；没写的文件，只有被引用到的那些才会。

今天（2026-09-08）在 `src/` 上的读数，**数法写在数后面**：

- **607** 个 `.zig` 文件。
- **A = 45**：文件自己含 `refAllDecls(@This())` 或 `refAllDeclsRecursive(@This())`
  ⇒ 声明一定被求值。
- **B = 110**：被别处以 `_ = <命名空间>;` 摸过、但自己没有 A ⇒ **没有保证**。
  `src/os/i18n.zig` 出事时正在这一类。
- **C = 452**：其余。⚠️ **这 452 个的覆盖程度没有测过，不要当成「已排除」。**
- 全 `src/` 内没有第二处文本引用的容器级声明 **114** 个（`fn` 74、`const`/`var` 40）。
  数法：剥掉 `//` 注释与字符串字面量后，缩进为 0 的 `const`/`var`/`fn` 声明名，
  在整棵 `src/` 里作为标识符出现的次数为 1（即只有它自己那处）；`export`/`extern`
  的不算，因为导出符号一定被分析。
  ⚠️ **这个数两头都不准**：多算 `@field` 取用的、通过类型间接用到的、只被
  `build.zig` 用到的；少算「只被另一个从没编译过的声明引用」的传递性死代码。
- **114 ∩ B = 4**，即与 `locales_map` 处境完全相同的声明：
  `src/lib/allocator.zig:6 convenience`、`src/os/macos.zig:9 isAtLeastVersion`、
  `src/os/macos.zig:50 SetQosClassError`、`src/renderer/shadertoy.zig:429 test_focus`。
  这 4 个逐个真编过一遍，**都编得过**。

**给 B 类文件补 `refAllDecls` 会不会抖出别的红，今天没有任何读数说它安全**，
所以没有做，也没有为此加新的检查脚本。
