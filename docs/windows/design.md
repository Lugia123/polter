...
| S4 其余 | 13,089 | 0.7 – 1.0 | 9,162 – 13,089 | 以 S1 的 0.88 为中心；**有一个已量出来的盲区，见 §1.7**；**已拆成十二块，见 `s4.md`（自底向上合计 8,400–11,900）** |
...
### 1.7 系数模型看不见的东西：五项，**而最后一项不是盲区**

> **这一节的标题 2026-09-01 改过。** 原来叫「S4 的盲区量掉了：三个候选里两个是我判错的」。
> 后来加进来的第四项（标准库差异）仍然是盲区，**而第五项不是**——
> 它的工作量确实接近零，被遮住的是另一样东西。
> **把它塞进「盲区」会让人去找一笔不存在的工作量。**
>
> 现在这一节里：**三项是判错的候选**（右键菜单、拖放不是盲区；无障碍是），
> **第四项是真盲区**（标准库），**第五项是另一种东西**（可发现性）。
...
#### ⭐ 第四类盲区：**标准库差异**，2026-09-01 在 UI-F 上撞到
...
#### ⭐ 第五项：**`MainMenu.xib` —— 不是盲区，是两件事被同一个 0 遮住了**

**先给数，因为这一项的全部意义在数里。** 2026-09-01 逐条量的：

| | 数 | 怎么来的 |
| --- | --- | --- |
| `MainMenu.xib` 的 `<menuItem>` 标签 | 103 | 用 `xml.etree` 数的。**先用正则数漏了 32 项**——嵌套子菜单把块吞了 |
| — 分隔符 | 23 | |
| — 子菜单标题（`File` / `Edit` / `View` …） | 11 | |
| **真正可点的命令** | **69** | 「103 个菜单项」这个说法**高估了 49%** |
| 核心自己发布的命令 | **96** | `command.zig` 的 `actionCommands`；与真机 `[palette] loaded 96 commands` 逐字相符 |
| **其中 69 条菜单项里已经能在命令面板找到的** | **49（71%）** | 逐条判的 |

**前四类说的都是同一件事：一段工作在 macOS 侧计为 0 行，而 Windows 侧必须写。
这一项不是。**

**Windows 侧要补的工作量确实接近零**——那 69 条里 49 条今天就能用，
剩下 6 条核心有 action（`command.zig` 里 30 个 action 显式 `=> comptime &.{}`），
14 条是 macOS 应用惯例（`Hide Others` / `Bring All to Front` / `Use as Default`…），
**属于 §1.3 那张「Windows 上没有这个概念」的清单**。真正的缺口只有三条。

**被遮住的是另一个维度：可发现性。**

> **一个 0 行的估算把「要写多少代码」和「用户找不找得到」压成了同一个数。**
> 前者确实是 0，后者不是——**而模型里没有后者这一栏，所以它读起来像「这一块没有工作」。**

##### 这个维度上的缺陷，不需要任何代码写错就存在

那 69 条里有 **20 条只是换了个名字**：

| macOS 菜单里的词 | 核心的命令名 |
| --- | --- |
| Find… / Find Next / Hide Find Bar | Start Search / Next Search Result / End Search |
| Copy / Paste | Copy to Clipboard / Paste from Clipboard |
| Zoom Split | Toggle Split Zoom |
| Select Split Above | Focus Split: Up |
| Return To Default Size | Reset Window Size |
| Preferences… | Open Config Using OS editor |

**用户在命令面板里搜 `Find` 找不到搜索功能。** 功能是对的、面板是对的、
核心发布的 96 条一条不少——**只是用一个用户不会想到的词命名。**

**而我们所有的验收都在问「这个功能对不对」。** 那次是用户十分钟撞出来的，
不是任何一条判据发现的。

##### 排查这一项要问的问题，和前四类不一样

前四类问的是「这一条在 macOS 侧一行都不用写的原因是什么」。
**这一项要问的是**：

> **这份工作在 macOS 侧是零行，是因为不用做，还是因为它做在了别处而那个别处没有行数？**

`MainMenu.xib` 是「做在了别处」：**103 个菜单项、0 行 Swift。**
而 macOS 用户能找到那 96 条命令，靠的不是命令面板，**是那个 0 行的东西。**
...
- **主菜单栏本身**：~~同上，不做。那 103 条命令通过命令面板（S3 块 E）和右键菜单到达。~~
  → **2026-09-01 用户当面推翻，改为要做**，见 `s4.md` §3。
  **这条裁决错在它回答了另一个问题**：「要不要一条横贯窗口顶部的 menu bar」答案确实是不要，
  但用户问的是「那 96 条命令，一个没被告诉过的人怎么找到」——**正是这一节自己刚量出来的
  那个维度**。同一份文档里隔了三十行，把「模型里没有这一栏」用「不做」填平了。
  Windows 侧的形态是标签条最左端一个菜单按钮 + 多级弹出菜单，估 700–950 行。
