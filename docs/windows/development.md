...
Windows 不能直接执行 `.sh`，报 `error.InvalidExe`——真机上那 9 个插件测试全挂在这。

**决定：8 个各补一份 `.ps1`/`.cmd`。** 曾经考虑过的另外两条：要求用户装
...
是插件的属性，不是宿主的推断。** 而这 8 个恰好都需要多系统支持，所以各补一份是本分。

**由此引出一个当前缺失的机制，和这 8 个脚本是两件事：**

| | 什么 | 什么时候 |
| --- | --- | --- |
| 现在 | 8 个插件各补一份 Windows 脚本 | M5 |
| **以后** | **插件清单里声明支持的系统，宿主按声明决定加不加载** | 未排期 |

第二条是**机制的缺口**：今天没有任何地方能表达「这个插件不支持 Windows」，
所以一个平台专属的插件只能在启动失败时才暴露，而那时它已经在用户的失败清单里了。
这不是这 8 个插件的问题，是插件模型缺一个字段。
