...
4. **上下文必须建在一个已经是最终尺寸的窗口上。**【实测，两个变量已拆开】
   ...
   **所以：栅格跟随 resize，画布不跟随。** 而 GL 的原点在左下，旧画布因此沉在
   左下角，空出来的正是**右和上**——用户报的就是这两条边。

   **一个缺陷，三处症状**，此前被记成三件不同的事：

   1. 最初的黑屏（窗口建成 100×100 → 画布永远 100×100）
   2. `--selfresize` 判「resize 追得上」（**中心像素永远被旧画布盖住**，见 §6）
   3. 全屏后右上两条黑边

   **上面那条规则因此不是解决方案，而是唯一让它看起来能用的那件事。**
   `tabs.rs:505` 的注释说「渲染器在 `surface_new` 里从这个 HWND 的
   `GetClientRect` 给自己定尺寸」——**那句是对的，它只是没说「而且此后再也不看第二眼」。**
...
>
> ⚠️ **这个日期未能确认**：全文其余日期都是 2026-09-01。可能是跨了午夜（测试机
> 是 +08:00，日志里出现过 03:34），也可能是笔误——**写的人也不确定，所以记成不确定，
> 而不是留一个看起来确定的数字。**
>
...
这七条都是真踩到的，而且它们是**同一类东西：工具沉默地给你一个看起来合理的错误答案**。
不是报错、不是崩溃——是一个能读、能信、而且是错的结果。这次移植里这类事出了七次，
