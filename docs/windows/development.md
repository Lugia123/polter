...
两者差一个固定偏移。**不要拿 `r=` 直接把它们 join 起来。**

**开关同时改一件事**：`[rthread]` 心跳平时每 8 次唤醒才打一行，
⇒ **停死的那一次只有 1/8 会打出自己那行**，其余时候日志最后一条属于一次
已经完成的唤醒，读起来一切正常。开关打开后每次唤醒都打，
「最后一行的 `wakeup` 比 `completed` 大 1」这个判据才成立。
**默认包里判据只能是「`wakeup` 这个数还在不在涨」。**
