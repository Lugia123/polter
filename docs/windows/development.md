...
**背景**：`design.md` §4.3 把 UI 装饰放在 S3 档，系数区间 0.15–0.6，
用户 2026-09-01 裁决取 **0.6 端（接近 macOS 的精细度）**，折合约 **2,665 Rust 行**。
...
> ⚠️ `design.md` §4.3 写的是 4,441，**高了 79**：插件视图那一格当时写的是估计值
> `~700`，实测是 621。分档表里其余各格与实测一致。

---

### 块 A · 窗口外壳与标题栏（macOS 1,447 → 估 450–600）
...
| 批 | 内容 | 估 | 为什么是这个优先级 |
| --- | --- | --- | --- |
| P1 | 搜索浮层（macOS 197） | 210–280 | 终端必需功能，不是装饰 |
| P1 | 键序列指示器（macOS 186） | 110–150 | **有 chord 键位就必须有**：否则用户按下前半个键位后没有任何反馈，无法判断自己在什么状态。Win32 稍便宜——不需要 `KeyCap` 那种声明式样式层 |
...
`design.md` §4.3 的 S3 是 4,441 × 0.6 = **2,665**，落在这个区间的上半。
**但这是巧合，构成对不上：**
...
>
> **这个盲区不限于 S3。** `design.md` §4.3 的 S4（13,010 行 × 0.7–1.0）用的是同一个模型，
