...
不证明「编进去的是我写的那几个测试」。** 便宜的补法是直接在 exe 里找测试名：
...
4. **上下文必须建在一个已经是最终尺寸的窗口上。**【实测，两个变量已拆开】
   ...
   **可见性不在这条里，这是量出来的不是想出来的。** 一次只把窗口留在隐藏、
   ...
   ### ⚠️ 2026-09-01 追记：**这条不是一条约束，是一个缺陷的症状**

   上面那段写着「还没查清的是机制……**判据很便宜：拖一次窗口看终端跟不跟**」。
   **那个判据一直没有人跑，而今天跑了，是阳性的。**

   实测（全屏 1440×900，以及退出全屏后分六步各 +60×25、每步间隔 500 ms
   放大到 1360×850）：

   ```
   painted = 1000x670   ← 始终是 surface_new 那一刻的客户区
   全屏时   right_black = 440   top_black = 200
   分步后   right_black = 360   top_black = 150
   ```

   **六次分开的、慢速的 resize，画布一次都没动。** 而同一时刻终端的**栅格是对的**
   ——在全屏的终端里跑 `mode` 得到 143×41，正是 `(1440−4)/10` 和 `(870−4)/21`。
   **这个读数来自 pty 的另一端，是这条线上唯一一个不是我们自己打的日志。**

   **一个缺陷，三处症状**，此前被记成三件不同的事：

   1. 最初的黑屏（窗口建成 100×100，此后屏幕上永远只有 100×100 那块）
   2. `--selfresize` 判「resize 追得上」（**中心像素永远落在那块旧矩形里**，见 §6）
   3. 全屏后右上两条黑边

   **上面那条规则因此不是解决方案，而是唯一让它看起来能用的那件事。**

   ### ⚠️ 同日再追记：**上面那句「画布不跟随」是错的，而它错在从症状推机制**

   写下它时的依据是「六次 resize 之后画的还是 1000×670」。**那是一个症状的稳定性，
   不是一个机制。**「六次都这样」证明的是它可复现，不证明它为什么。

   插桩之后，`drawFrame` 里四个量一行打出来，**整条链是通的**：

   ```
   [win] surface WM_SIZE 1240x790 -> set_size (#1)
   [rsz] surface=1240x790 screen=1000x670 changed=true  needs_redraw=true
   [rsz] target=1000x670  screen=1240x790 stale=true
   [rsz] surface=1240x790 screen=1240x790 changed=false needs_redraw=true
   [rsz] target=1240x790  screen=1240x790 stale=false
   [hud] size 124x37 from client 1240x790 cell 10x21
   ```

   `set_size` 到了、`screen` 跟上了、`target` 重建了、重绘发生了、栅格也对。
   **两个人事先各写了一个预测（`changed` 恒 false / 早退重贴陈旧目标），两个都错。**

   **而像素仍然是黑的。** 用一条不共享失败形态的探针核过：截一张跨分界线的原图，
   左边终端背景、右边纯黑，**分界落在客户区 x=1000 —— 正是旧宽度**。

   > **准确的说法是：尺寸全对，而只有旧尺寸那块矩形上了屏。**
   > 和「画布不跟随」在肉眼上完全一样，**在代码里差着整整一层**——
   > 前者指向尺寸传递，后者指向 present/blit 的矩形。

   **三处症状仍然是一个缺陷**，只是那个缺陷的位置往下挪了一层：不是 `size.screen`
   或 `frame.target` 没更新，而是**上屏的那个矩形停在了创建时的尺寸上**。
   `tabs.rs:505` 的注释说「渲染器在 `surface_new` 里从这个 HWND 的 `GetClientRect`
   给自己定尺寸」——**那句仍然是对的，只是它描述的那件事发生在比我们以为的更下游。**

   **下一个探针要打矩形，不要打尺寸**：`glViewport` 的四个参数、blit 的源矩形与
   目标矩形，外加 pane 标识。`[rsz]` 那五个量**全是尺寸和布尔，所以对位置完全是瞎的**
   ——它能证明尺寸对，证不了矩形对，**而它缺的那一维恰好就是剩下的那个问题**。

   **缺陷在 `src/renderer/`，宿主侧没有欠账**：`set_size` 调了、窗口真的变大了、
   WGL 上下文绑的就是那个窗口（三个句柄逐个核对过）、栅格因此正确。
   **而 `generic.zig` 是共享核心**，GTK 和 macOS 都走它——所以修之前要先回答
   「GTK 上为什么没这个病」。如果答案是「GTK 每次 resize 都重建整个 GL area」，
   那么修法可能是补一条 Windows 独有的通知，**而不是改共享逻辑**：另外两个平台
   现在是好的，改共享逻辑要承担让它们回归的风险。
...
### 5.4.0 五次回填，统一到一个口径（2026-09-01 重算）

**此前四次回填用了三种单位**（总行 / 代码行 / 含测试的代码行），而其中两个百分比正被
用来论证「系数偏低」——**用来判同一个系数的两个数据点，量的不是同一个东西**。

**统一口径：代码行（去空行、去注释）减去 `#[cfg(test)]` 块。** 理由是估算本身就是
从 macOS **代码行**（`design.md` §1.3 的 4,362）乘系数来的，比较的两边必须同单位。

| 块 | 估算 | 实测 | 偏差 |
| --- | --- | --- | --- |
| A 窗口外壳与标题栏 | 450–600 | **292** | **低 35%~51%** |
| B 标签条交互 | 280–360 | **686** | 高 91%~145% |
| C-P1 搜索 + 键序列指示器 | 320–430 | **750** | 高 74%~134% |
| C-P2 只读徽标 + 缩放标牌 | 130–180 | **308** | 高 71%~137% |
| E 命令面板 | 300–400 | **534** | 高 34%~78% |
| D 分屏分隔线 | 160–220 | **359** | 高 63%~124% |
| G 下拉终端 | 380–550 | **441** | 落在区间内 |
| F 插件与设置页 | 300–420 | **1,234** | 高 194%~311% |
| **八块合计（S3 全档）** | **2,320–3,160** | **4,604** | 高 46%~98% |

**⚠️ 换口径把块 A 的结论翻了过来。** 按总行（500）它落在 450–600 内，
按统一口径（292）它**低于下界 35%**——而先前「块 A 落在区间内、所以偏高不是系统性的」
那个论证，正是建在总行那个数上的。

**所以现在的形状是六高一低，而不是「三高一中」。** 低的那一块是唯一一块
**范围从头到尾没变过**的；其余六块各自都在估完之后长过范围
（溢出策略是裁决后加的、改名要真 IME、搜索要跨线程收件箱、徽标要三态区分、
分隔线要有生命周期管理、下拉终端的全局热键是交付时才加计进去的 80–150）。

### 这不是六个证据加一个反例，是一个证据

**范围变过的那六块和它们自己的估算不可比**——它们不是「支持系数偏低的六个点」，
也不是「支持范围变化的六个点」，**它们是六个没有资格进入这个论证的观测**。

**所以关于 0.6 这个系数，我们有 n = 1，而且那一个点说估算偏保守（低 35%~51%）。**
这比换口径之前看起来的位置弱得多：当时是「四个高点被范围变化解释掉」，
现在是「只有一个点能说话，而它说的是另一个方向」。**结论仍然是不动 0.6，但理由变了。**

> **一个结论如果在你统一单位之后翻转，那它原本就不是被数据支撑的，是被单位支撑的。**
> 和 §5.4 那次「数对得上、构成对不上」是同一族——那次是构成错，这次是量纲错。

### 从下一块起：「范围变过」记账，不记是非

现在「范围有没有变过」是个是/否字段，而**是/否只能作废一个点，不能挽救它**——
上面六个脏点就是这么来的。

**改成记账**：估算时把范围写成一份**逐条清单**；交付时，对每个**估完之后才加进来的
条目**单独记下它花了多少代码行。于是
...
**脏点就能变回可用点**，而且「估歪了多少」和「范围长了多少」第一次能分开。

⚠️ **这个方法不能倒着用。** 块 D 事后归过一次账（`sync` 的窗口池 92 行、hover 与
捕获取消约 40 行、`split-tree` 的 `resize_at` 71 行都不在当初那三个词里，
扣掉约 200 行得 159，正落在 160–220 的下界附近）——**那个 159 不是证据**。
事后归账的人知道目标数是多少，**有把条目往「估得准」方向分的动机**。
**只有在动手之前写下的清单算数**，所以这一条从块 F 开始生效，不追溯。

**S3 全档收口：系数模型 2,617，实际 4,604，是模型的 1.76 倍。**

### 记账制的第一次使用，产出的是诊断不是数字

块 F 是唯一一块**动手之前写下了逐条清单**的（29 条，交付时 34 条）。三个数：

```
原估算        300–420
动手前清单    470          ← 拆到条目之后，同一个人，同一天
实际          1,234
后加条目      115          （最大两笔：`ACTION_OPEN_CONFIG` 的处理 35 行、Info/GString 绑定 30 行）
可比实际      1,119        ← 对清单仍高 138%
```

**关键在最后一行**：扣掉范围变化之后仍然高 138%，**所以这一块不是「范围长大」，是估歪了**。
而清单让作者能说出歪在哪：

> **我把每条按「画一个控件」估，而每条实际是「一个窗口 + 一组子控件 + 一条数据通路 + 一条日志」。**

**前六块只能说「超了」，这一块能说「为什么超」。** 这就是把是/否字段换成一本账买到的东西——
**不是让数字变好看，是让超支变得可归因。**

**不据此动 0.6**：n 仍然是 1，而且这一块的估算和交付是同一个人。
**可执行的结论是另一条**：估 UI 类工作时，先对每条问一句「**这一条要不要一个窗口**」——
要窗口的条目和画一个控件的条目差着一个量级，而它们在清单上长得一样。

> **这一节的表是重算的，而第一版算错过一次**：判断分支只处理了「高于上界」，
> 其余一律标成「落在区间内」，于是 292 被标成了落在 450–600 里。
> **一个只检查一个方向的比较，会把另一个方向的偏差报成正常。**
...
## 5.5 其余那一档（S4）的分解：见 `s4.md`

S3 拆成七块以后能派出去，S4 一直没拆，所以一直没开工——**不是难，是没人写那份文件**。
2026-09-01 夜补上，独立成篇：`docs/windows/s4.md`。
十二块 A–L，自底向上合计 **8,400 – 11,900 Rust 行**；
其中 **A（菜单系统）和 K（拖放）在系数模型里的预算是 0**，
因为它们在 macOS 侧分别是 0 行 Swift（`MainMenu.xib`）和四个协议方法。
**5.4 结尾把这两个列为「盲区候选」，现在它们是实测。**
...
这八条都是真踩到的，而且它们是**同一类东西：工具沉默地给你一个看起来合理的错误答案**。
不是报错、不是崩溃——是一个能读、能信、而且是错的结果。这次移植里这类事出了八次，
所以判断「成了没有」时，**先问工具凭什么知道，再看它说了什么**。

**第八条和前七条不同，值得先说**：前七条里工具至少参与了——它跑了、它说了话、
只是话是错的。第八条里**没有任何工具参与过**，因为那句话在注释里。

#### 输入法会吃掉字母键的按下，于是按键测试测了别的东西

```
[key] TSF ate msg=0x100 vk=0x41 (not dispatched)
```

中文输入法激活时，`WM_KEYDOWN` 被文字服务吃掉，只有抬起到得了我们。所以
**任何「发一个字母，看它有没有走通某条路径」的测试，在那个状态下都不成立**——
它不会报错，它会安静地测量另一件事。要用按键做判据，用回车或功能键，
或者先把 IME 的状态读出来写进日志。

这一条和另外三条是同一族，四条排在一起才看得清它们是什么：

- **判据盲于缺陷**：`--selfresize` 看中心像素有没有变黑，而一块贴在左下角的旧画布
  总是盖住中心点——那条判据在它该发现的缺陷面前是瞎的，**并且当天通过了**。
- **表写死了目标系统**：启动表如果直接读 `builtin.os.tag`，Windows 那几行在构建机上
  不可达，写错了没有任何测试能为它变红。**一张没有东西能反驳的表不是表，是一个愿望。**
- **自己的解析器只能证明自己自洽**：手写 JSON 加自己的单元测试，量的是「我和我同意」，
  而真正会出事的是两个独立实现不同意。
- **核对做在了另一个对象上**：脚手架脚本的字节数被反复核了三轮，而**过期的是被测的
  那八个插件文件**。检查在做，只是没做在该做的地方。

**前三条是判据够不到被判的东西，第四条是够到了、够的是另一个东西。**

第六条排在最前面，因为**它比其余五条都发生得早**：那五条讲的都是「构建说了什么」，
而它讲的是**这次构建里根本没有你的代码**。前五条没有一条能抓到它。

#### 你的文件不在编译单元里——那个绿色和你无关

接线一个新模块之后 `cargo build` 报 **0 error、0 warning**，而 `mod divider;`
**从来没有被加上去**：加它的那条 `A && B && C` 命令链断在一个找不到东西的 `grep` 上，
后面全没执行，退出码却来自最后一条跑过的命令。

**证伪的方法只有一个：把那个文件改坏，看构建是不是真的变红。**
第一次做这件事时，往 `divider.rs` 里塞一行语法错误，构建**仍然 0 error**——
那个绿色从头到尾没有编译过这个文件。修好之后重做同一次注入：**2 error**。

`grep -c 'mod divider' main.rs` **等于 1 不能替代它**。那行字在，不等于它生效：
它可能在一个 `#[cfg(...)]` 块里、文件名可能对不上、也可能有第二个同名模块遮蔽它，
**这三种情况 grep 都一样是 1**。同样地，一次成功的构建也不能替代它——
**上面那次就是成功的构建。**

这条对每一个新增文件都成立，包括本文档 §5.4 那七块里的每一块。
**代价是一次注入加一次构建；不做的代价是把一个从未编译过的文件当成已交付。**
...
#### 注释里那句「某处已经做了这件事」——唯一一个连工具都够不到的地方

写「打开配置文件」那颗按钮时，我在它旁边留了一句注释：

> `open_config` comes back as **an action the host already handles**

**那句话是假的。** `grep ACTION_OPEN_CONFIG windows/host/src/*.rs` 当时是 **0 命中**——
宿主根本没接那个 action。**按钮点下去会一路走到核心、发回一个没人处理的 action，
然后什么都不发生。**

**它藏在这个仓库里唯一一个可以说谎而不留痕迹的地方。** 注释不参与构建、不参与测试，
没有任何东西会为它变红。这一节前七条讲的都是「工具说了话，而话是错的」，
而这一条**没有任何工具说过话**。

##### 为什么前面所有的纪律都够不到它

负对照、变异地板、编译期断言、跨实现 fixture——**这些纪律的共同要求是「判据要能失败」。**
一句没有符号名的注释**在结构上无法失败**：它没有被求值，没有被链接，
没有任何输入能让它红。**它不是一个弱判据，它根本不是判据。**

##### 对策：**要么带上一个可以 grep 的符号名，要么不要写**

```rust
// ✗ 无法检验：没有任何东西能让这句话失败
// `open_config` comes back as an action the host already handles.

// ✓ 可检验：下一个人 grep `ACTION_OPEN_CONFIG` 就会撞见它是不是真的在
// Handled in `cb_action` under `ffi::ACTION_OPEN_CONFIG`.
```

**这不是文风要求，是把一个不可检验的断言变成可检验的断言。**
带符号名的假话，下一次有人搜那个符号时会被撞见；不带的永远不会。

同样的形状还有几种写法：`见 xxx.rs 的 yyy()`、`这条契约在 §N.M`、
`那个 bug 在 <commit> 修了`——**共同点是它们都能被人查，而「已经处理了」不能。**

##### ⚠️ 我是怎么发现它的：**运气，不是纪律**

**这一条必须原样记下来，否则这段会被读成「我们有办法抓到它」。**

我不是在审查那句注释时发现的。我是在**写那颗按钮的行为**时去 `grep ACTION_OPEN_CONFIG`，
想看看该往哪儿接——**然后才撞见 0 命中**。如果那颗按钮当时不需要我去查，
那句假话会随着提交进仓库，并且会一直在那里：**下一个读它的人会相信它，
因为它读起来完全合理，而且没有任何东西反对它。**

**把运气记成纪律，下次就不会再去查。** 所以上面那条对策的真实价值不是「帮你发现假话」——
**是让你写不出一句无法被发现的假话。**
