...
| **五块合计** | **1,480–1,970** | **2,570** | 高 30%~74% |

**⚠️ 换口径把块 A 的结论翻了过来。** 按总行（500）它落在 450–600 内，
按统一口径（292）它**低于下界 35%**——而先前「块 A 落在区间内、所以偏高不是系统性的」
那个论证，正是建在总行那个数上的。

**所以现在的形状是四高一低，而不是「三高一中」。** 低的那一块是唯一一块
**范围从头到尾没变过**的，高的四块各自都在估完之后长过范围
（溢出策略是裁决后加的、改名要真 IME、搜索要跨线程收件箱、徽标要三态区分）。
**这两件事仍然是两个原因，不是一个系数。**

**区间会被突破**：S3 全档系数模型是 2,617，**五块已经用掉 2,570**，
而 D（160–220）和 F（300–420）尚未开工——**按下界完成约 3,030，按上界约 3,210。**
