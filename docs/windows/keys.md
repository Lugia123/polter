...
**（b）同一个和弦被写了两次，后写的赢。**
`Binding.zig:2673` 的 `gop.value_ptr.* = .{ .leaf = ... }` 是无条件覆盖：
...
**反向表**，它按 **action** 存，而且 `Binding.zig:2627` 有一条：
...
| 窗口 | 源 | 吃掉的键 |
|---|---|---|
| 命令面板 | `palette.rs:747` | `Esc`（关）、`Enter`（执行）、字符输入 |
| 搜索栏 | `search.rs:497` | `Esc`、`Enter` |
| 设置页 | `settings_ui.rs:968,1316,1644` | `Esc` |
| 提示框 | `prompt.rs:492` | `Esc`、`Enter` |
| 覆盖层 | `overlay.rs:159` | `Esc`（转发给父窗口） |
...
- `Config.zig:6920-6924`：`close_window` 在 `!isDarwin` 块里绑的就是
  `.{ .physical = .f4 }, .mods = .{ .alt = true }`——**Windows 上确实有这条绑定**。
- 用的是 `set.put` 不是 `putFlags`，所以 **非 performable，进反向表**（`Binding.zig:2627`）。
- 全树只有一处 `physical = .f4`（另一条 `close_window` 在 mac 分支，绑 `Super+Shift+W`），
  **没有第二条绑定会来覆盖它、摘掉它的反向项**。
- 渲染这一半也核过：`mods_label(alt)` → `Alt+`（`keyseq.rs:121`），
  `key_label(physical, F4)` → `F4`（`keyseq.rs:529` 有断言）。所以标签**只能是** `Alt+F4`。

于是「关闭窗口」这一行是空的，只剩两种解释，处置相反：

| 观察 | 解释 | 处置 |
|---|---|---|
| 关闭窗口空，且**其余四行也全空** | 链断在更上游——`ghostty_config_trigger` 没导出、config 句柄是空、`shortcut_for` 根本没被调用。日志里会有 `[keys] ghostty_config_trigger not exported...`（`keys.rs:401`） | **前四行的结果全部作废**，先修链，再重跑整个第一部分 |
...
**第二部分（第一部分跑完、结果报回来之后再跑）：按下去做什么**（每步之后 `ui_snapshot` 读标签条**文字顺序**，
并 `findstr /c:"[action]" <日志>` 核对真正触发的 action 名。**不要用截图判断顺序**——
截图分不出「顺序没变」和「顺序变了但看起来差不多」。日志里的 action 名是判据，界面是佐证。）

| 步骤 | 按键 | 期望 | 失败意味着 |
|---|---|---|---|
| 1 | `Ctrl+Shift+→` | 焦点到标签 2，**顺序仍是 1,2,3** | 顺序变了 → `move_tab` 还在生效，宿主表没删干净 |
| 2 | `Ctrl+Tab` | 焦点到标签 3，顺序不变 | 同上 |
| 3 | `Alt+1` | 回到标签 1 | `goto_tab` 挂了 |
| 4 | `Ctrl+Shift+PageDown` | **顺序变成 2,1,3**，焦点仍在原标签上 | 只切了焦点 → `move_tab` 没生效，(a) 的读法错 |
| 5 | 选中一段文字，按 `Shift+Insert` | 粘进来的是**刚选中的那段**（选择剪贴板），不是系统剪贴板 | 粘进来的是系统剪贴板 → `paste_from_selection` 没覆盖成功，(b) 的读法错 |
| 6 | `Ctrl+Shift+W` | 当前**标签**关闭，窗口还在（剩两个标签） | 整个窗口关了 → 生效的是 `close_surface`，(b) 的读法错 |

第 6 步的日志原文单独报回来：它同时是任务 127（关最后一个标签进程不退）的证据。
