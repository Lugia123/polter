...
打开菜单，`ui_snapshot` 读「编辑」和「文件」两组的行文本原文，回报三件事：

| 菜单行 | 我的预测 | 预测错了意味着 |
|---|---|---|
| 复制 | 显示 `Ctrl+Insert`（**不是** `Ctrl+Shift+C`） | performable 绑定其实进了反向表，3.3(b) 的读法错 |
| 粘贴 | **不显示任何快捷键** | 反向表的摘除逻辑与我的读法不同 |
| 关闭标签 / 关闭 | 至多有一行显示 `Ctrl+Shift+W` | 两行都显示 → 反向表里两个 action 共用一个和弦 |

**第二部分：按下去做什么**（每步之后 `ui_snapshot` 读标签条**文字顺序**，
