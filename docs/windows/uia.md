...
### 事件收不到时，那条锁序**仍然可以单独验** —— 但它盖不到的东西要说清

真机上出现过一种情况：**订阅成功、投递为零**（一个 20 行、不经过我们代码的最小探针，STA、交互式控制台、`DoEvents` 泵 20 秒、真实点击两次，仍然 0）。UIA 的「拉」在那台机器上一直是好的（四个 dump 模式、读 Document 的 Value 全都拿到数据），**坏的只有「推」**。

这时 `-Mode events` 那一格里最要紧的半格**仍然是可测的**，因为它问的不是「客户端收不收得到」，而是**「我们发的时候会不会自锁」**：

> 有客户端订阅着的情况下做那四个动作，**宿主不挂、不崩、日志继续长**。

宿主为此在每次通知上打三段（**判据请按这些字符串抓，它们是从源码里取出来的，不是凭记忆写的**）：

| 时机 | 行 |
| --- | --- |
| 决定不发 | `[uia] <什么> not announced (clients_listening=<真假> window_live=<真假>)` |
| 就要发 | `[uia] raising structure change` / `raising focus change for tab N` / `raising name change for tab N` |
| 发完了 | `[uia] structure change raised` / `focus change raised for tab N` / `name change raised for tab N` |

**两行夹住那个调用，是因为自锁不留栈也不留消息，它留下的是一份停住的日志。** 只有一行的话，停住的日志说不出停在哪；有这一对，**日志停在 `raising` 而永远走不到 `raised`，就点名了那个调用** —— 那是「一轮找到」和「一夜找到」的差别。

`clients_listening` 那个值也是打出来而不是推断的：`UiaClientsAreListening` 的语义是粗的，而「注册成功但投递为零」恰好是它两种可能读法分岔的地方。**与其从记忆里定它算哪一种，不如让日志说这次走了哪条分支。**

**这半格盖不到什么，逐条：**

- **不证明客户端收得到** —— 那正是这台机器上坏掉的那一半。
- **不证明事件类型对**（该发属性变更的地方有没有误发结构变更）。
- **不证明发在正确的元素上**（RuntimeId 对不对、TabList 和根有没有都发到）。
- **`clients_listening=false` 时这半格什么也没测** —— 那条回调路径根本没走。**读到那一行就说明这次跑不算数**，要先让一个客户端订阅上。

**上面三条只有真正收到事件才能验。** 所以这半格是**把「完全不能验」变成「能验其中一半」**，不是把这一格验完了。
