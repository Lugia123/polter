...
总管挂了一夜、开了三四个群之后，`group_list` 给它的是这样一串：

```text
build, research, nightly
```

这三个字是谁起的？分别在干什么？**总管自己起的名字，它自己也认不出来了** —— 一个跑了八小时的会话，早期的上下文已经被冲掉了。而它恰恰需要在这个时候判断「哪个群还需要盯着」。

### 设计

每个群带一个 `brief`：一段自由文本，总管自己写、自己读。

| | |
| --- | --- |
| 谁能写 | **只有总管**（`group_set_brief`） |
| 谁能读 | **只有总管**。`group_read` 不返回它，成员看不到 |
| 什么时候读 | `group_list` 直接带上，总管一眼看到每个群在干什么 |
| 程序怎么用 | **完全不用。**存进去，取出来，不解析 |

为什么成员看不到：这不是给群里的人看的公告，是总管给自己的备忘。写给自己的话和写给别人的话不是一回事 —— 强行让它兼任对外说明，会让总管在写的时候开始斟酌措辞，而备忘的价值恰恰在于可以草率。
...
| 禁令 | 群任务说明 |
| --- | --- |
| 不提供任务 CRUD | 不提供。它是群的一个字段，不是可增删查改的任务实体 |
| 不持久化任务内容 | **这一条要修订** —— 见下 |
| 不解析任务描述 | 不解析。程序把它当不透明字符串，从不读内容 |
| 不做排序或依赖 | 不做。没有状态、没有优先级、没有完成判定 |

第 2 条的原文是「不持久化任务内容」，而群任务说明确实会随群一起存下来。**修订为：不持久化任务的结构。**

区别在于：一段自由文本没有状态机，没有「未开始/进行中/已完成」，没有依赖关系，没有负责人字段。它不能被查询、被排序、被统计。它对程序而言和聊天记录里的一句话没有区别 —— 事实上它本来就可以写成群里的第一条消息，做成字段只是为了它不被后面几百条冲掉。

**判据一句话**：如果哪天有人想加「把 brief 标记为已完成」或者「按 brief 排序群」，那就是越过了 R8，应该拒绝。存一段字不是管理，给这段字加状态才是。
...
| `group_list()` | 无 | 全部；**但 brief 只对总管出现** |

`group_list` 的返回因调用者而异，这是工具面里的第一处。理由与 `history: none` 同源：可见性是按人算的，不是按数据算的。

### 与建群的关系

**建群时应当就写。**skill 里要求总管 `group_create` 之后立刻 `group_set_brief` —— 那是它对这个群的意图最清楚的一刻。等到八小时后想起来要写，它已经想不起来了，而那正是需要这段字的时刻。
