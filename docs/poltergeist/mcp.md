...
> 最后更新对应的 git commit：`601026f3c`（`601026f3cb28c66c47d8ea52dd811943c42e8874`）
...
`quiescence_ms` 一模一样——`0` 会被读成"量过了，此刻就是零"，恰好是"没在量"
的反面。
...
2. **卸任之后自己升不回去。**`addSupervisor` 只在键位那条路上，工具面没有。
   所以这个动作对 agent 是单向的——这也是为什么 skill 里写的是"活干完了才
   卸任，不是安静了半小时就卸任"。
3. **用户可以完全禁掉**（`poltergeist-supervisor-stand-down = false`）。
   此时返回 `StandingInstruction` 而不是 `NotPermitted`，措辞和无限工作模式
...
| **选**：关闭插件只能由用户做               | 总管收拾不了一个吵闹的通知渠道                                                          | 通知是挂机时用户**唯一**的知情通道，能掐掉它等于能给自己关灯；与 `set_work_mode` 那条"不得解除用户常设指令"同形     |
