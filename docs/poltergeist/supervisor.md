...
挂机过夜的下一天早上，用户会关掉 Polter 再打开。现在这一下会丢掉全部监督关系、全部群、以及每个群在干什么 —— 聊天记录还在磁盘上（见 [chatui.md](chatui.md)），但那是一份日志，不是可以恢复的状态。

### 难在哪：终端的身份是随机的

`Surface.id` 是启动时现摇的随机 u64（`src/Surface.zig` 的 `init`）。它足够做进程内的寻址，但**跨重启没有任何意义**：上次的总管是 `0x9491…`，这次不会再有这个号。

所以「把 id 存下来下次读回」这条路根本不通。需要的是一个**跨重启仍然指向同一个终端**的标识。

### 已核实：macOS 上存在这样一个标识

`SurfaceView` 有一个 `uuid`，而且会话恢复时是**读回来的而不是新生成的**：
...
也就是说，只要 `window-save-state` 开着、Ghostty 恢复了上次的窗口，**同一个终端在两次运行之间是可以被认出来的** —— 只不过这个标识在 apprt 侧，核心不知道。

（未核实：GTK 侧有没有等价物。GTK 的会话恢复本身就未验证过。）

### 设计：让 apprt 报告持久标识，核心不发明第二套

```text
apprt 知道「这个 surface 是上次那个」   →  报告一个持久 key
核心存快照时用这个 key，而不是 Surface.id
下次启动，apprt 报告同一个 key         →  核心认出它，还原角色
```

这与 [mcp.md](mcp.md) 的「不引入第二套 ID」是同一条原则的延伸：**身份仍然由宿主给出**，核心只负责记住它和角色的对应关系。没有报告 key 的平台或没开会话恢复时，key 为空 —— 那种情况下这个终端就是新的，如实当作新的。

### 分成三层，因为它们能还原的程度不同

| 层 | 内容 | 能自动还原吗 |
| --- | --- | --- |
| **对话** | 群、成员、每个群的任务说明、聊天记录 | **能。**群不依赖某个终端还在 —— 成员走了群还在，这本来就是群的语义 |
| **关系** | 谁是总管、谁被监视、工作模式 | **在有持久 key 的平台上能。**否则降级 |
| **瞬时状态** | 静止时长、rounds 计数、收件箱 | **不还原，也不该还原。**静止了多久是关于「现在」的度量，重启之后从零开始才是诚实的 |

第三层要说清楚：把 `rounds` 存下来读回，等于宣称「昨晚数到 3 次，今天接着数第 4 次」，可昨晚那个进程早就没了，被监督的 agent 也重启了。**下班模式的「连续 n 次」跨重启没有意义**，硬要延续只会让 n 变成一个说不清的数。

### 降级：认不出来就问，不猜

在拿不到持久 key 的情况下（GTK、关掉了会话恢复、或者用户手工开了新终端），Polter **不猜**。它保留上次的关系快照，并在群聊里留一条给总管：

```text
[poltergeist] 上次有 3 个被监视终端：✳ Write retry.py、✳ Write test_retry.py、reviewer。
这次还认不出它们，需要用户重新指定。
```

按工作目录或标题去猜「这大概是上次那个」是能做的，但不能做：真机上三个 agent 终端的工作目录**完全相同**（都在同一个仓库里），标题也会随任务变。猜错的后果是把 A 的监督关系安到 B 头上，而这种错误没有任何提示 —— 它看起来一切正常。

### 存在哪

`$XDG_STATE_HOME/polter/session.json`，0600，与聊天日志同一个目录。

写的时机：**每次关系变化后立刻写**，而不是退出时写。退出时写有一个致命前提 —— 进程正常退出。而挂机过夜恰恰是最可能崩溃、被系统杀掉、或者被强制重启的场景。一次关系变化的写入是几百字节，不值得为它省。
