...
**本设计选择：默认降级为总管代理决策**，每次降级都写进审计日志并在群聊里留一条。

理由要写足。R5 的场景二本身就是「人不在，机器继续」；窗口外发生确认请求，正常语义就是「人不在」。把「人不在」解释成「停下来等」，等于让整套东西恰好在最需要它的时段失效——排队方案会让被监督终端整夜卡死，早上 09:00 一次性爆出一堆积压。

**逃生阀**：允许用户把某个被监督终端单独标为「必须人确认」，这类请求在窗口外一律排队、绝不代理。关键在于「排队 vs 代理」是**按终端**二选一，不是全局二选一。取舍见 T5。

**与 R2 缝合的一句**：降级为代理决策不会导致自动点 yes。总管代理的只是「要不要继续 / 要不要换任务」这类调度决策，而「替对方按下工具授权的 yes」根本不在能力集里——见下一节。
