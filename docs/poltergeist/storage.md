...
> `ChatLog.history`、`group_history`、`ChatLine.log_seq`；常驻存档的宿主一侧见 `src/poltergeist/Archive.zig`、
...
| | 路径 | 给谁 | 边界 | 谁读它 |
| --- | --- | --- | --- | --- |
| **流** | `chat/chat.jsonl`（+`.1`） | 机器 | 8MB 轮转，留两代 | `Archive`/插件按 seq 往后跟 |
...
只落一条 warn。理由：流是插件跟读的那一份、也是发 seq 的那一份，为了「更好读」
