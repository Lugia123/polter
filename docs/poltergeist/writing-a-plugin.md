...
- **`key` 写成和目录名一样。** 宿主不校验这一条，但查重按目录名，而下游全按
  `key`——设置文件是 `<key>.json`，宿主查重也按它。不一致的后果是一个插件
  的设置文件叫着另一个名字。
...
（`plugin_test` 对 `archive` 类不启动任何东西：它已经在跑，起第二份只会得到
第二个订阅者，把每条消息都存两遍。回来的是正在跑的那个的近况，那才是「为什么
什么都没存档」的答案。）
...
权威是 `src/poltergeist/Archive.zig` 和 `Feed.zig`，[storage.md](storage.md)
...
- `hello` 是协议版本号，**是数字而不是标记**，这样不认识这个版本的插件可以直接
  退出，而不是去猜后面那行的形状。
- `cursor` 是**这一次运行之内**宿主已经确认到哪了。**新起一次 Polter 时它恒为
...
### 3.3 重启之后怎么续上：**它不续**，而这不是缺陷

**没有游标文件。**`<key>.cursor` 和 `Cursor.zig` 都不存在了。宿主不记你存到
哪，因为宿主已经不再是你的数据源：你订阅的是**从订阅那一刻起发生的事件**
（`Feed.zig`），核心的日志文件你根本不打开。

于是三条实际后果，写插件时会立刻撞上：

1. **Polter 没在跑的时候发生的事，你拿不到。** 也没有任何接口能补。核心自己
   的记录是完整的（`chat/<群>/<日期>.jsonl`），但那是给人和 AI 读的，不是给
   插件补课的。
2. **你落后太多会丢。** 你的订阅队列是有界的（默认 4096 条 / 4MB），满了丢最
   老的并计数，宿主会 warn 一行。所以**别在确认的路径上做慢事**。
3. **重复仍然是常态**，见下一节——**这一条一点没变**，而且现在它是你唯一的
   去重手段。

握手时你**仍然可以**回一个 `cursor` 说明自己的状态（「我的库已经到 5000」）。
宿主会记一条 info，然后什么都不改：它没有东西可以重放。**往哪个方向都不是
失当**——每次 Polter 重启，你的库都必然跑在宿主的 0 前面。`archive.py` 现在
这么做仍然安全，只是宿主不再照着回拨。
...
才能恢复，等于「存档坏掉的时长正好等于没人看着的时长」。**在这期间已经交给你
但你没确认的那些事件仍在订阅队列里**，重起之后原样再发一遍；而这期间新发生的
消息，超出队列上限的部分你会丢掉——核心自己的记录一条不缺。
...
- **两个目录声称同一个 `key`** → 后一个被跳过并警告。两份拷贝会共用设置，而且会各存一遍。
