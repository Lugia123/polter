...
| `status=provisioned` | 真写了东西 | |
| `status=unchanged` | 二进制在，该有的都已经在 | **每次启动都说**，见下 |
| `status=failed step=<mcp\|skills>` | 二进制在，某一步没成 | **通知**，说清哪一步 |

只有 `failed` 会主动找到人，走 `polter_tell`，所以它必须写在本批次的应答之前。

⚠️ **`unchanged` 是第四态，它替换掉的是「没写就不出声」（任务 218）。**
那条规矩的理由是真的——每台主机每次启动一行，日志里就是八行空话。但**沉默本身
是缺陷**：无事可做的插件和「起来了、还没干活就倒了」的插件，在日志里都是
`started` 之后再无下文。实测有插件在真写了配置那一次报了 `provisioned`，**之后
五次启动全是静默**，而从日志里分不出它是没事干还是已经不工作了。
那八行不是空话——它们正好回答了读日志的人真正的问题：**「查过了，都在」还是
「根本没走到那一步」**。
