...
> 状态：写作规范，与实现进度无关。各章自己的状态见各章开头。
> 一处需要注意：本批文档写作时 Poltergeist 尚未实现，所以行文里「设计示意，仓库中尚不存在」这类话在若干处已经过时。凡与代码冲突，以代码为准。
...
5. 本批文档写作时描述的是**尚未实现的设计**，所以行文用「应」「拟」「本设计选择」等设计语气。现在大部分已落地：已实现的部分应改写成陈述语气并给出 `路径:行号`，仍未实现的保留设计语气并显式标注。两者在同一段里必须一眼分清。
...
1. 第 1 行 `# <标题>`。
2. 引用块三行：`> 最后更新对应的 git commit：` + 短哈希 `f81dcadc8`（全哈希 `f81dcadc82ea2afdcf2dc92929037701122f05b5`，2026-08-14）；`> 校验方式：` + `git log -1 --format='%H %h %ad %s'`；`> 状态：…`（各章按自己的实现进度写，不要照抄）。
3. `## 本章覆盖什么`（3–8 条）。
...
- **设计示意的伪代码 / 配置样例不是源码引用**，必须在块上方写明它是示意还是现状。已落地的要指向真实文件，未落地的要写明「仓库中尚不存在」。
