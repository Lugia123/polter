...
> 状态：**已实现**（S3，macOS 侧）。窗口在 `macos/Sources/Features/Poltergeist/`，取数的 C ABI 在 `src/apprt/embedded.zig`，快照 API 在 `src/App.zig`。GTK 侧仍未做。
> 本章前半是选型推导，保留下来是因为它解释了**为什么**是这个形状；实际落地的样子见「实现」一节，两者不一致时以代码为准。
...
窗口是 `NSWindow` 加一个 `NSHostingView`，用代码搭而不是 nib —— 它只装一个视图，nib 会变成一个需要同步维护的文件而换不来任何东西（`macos/Sources/Features/Poltergeist/PoltergeistChatController.swift`）。

左栏是**群列表**（不再有私信线程，私聊已在模型层取消，见 [mcp.md](mcp.md)），右栏是选中群的消息流加发言框（`PoltergeistChatView.swift`）。发言者用 `Surface.id` 标识，与 [tabs.md](tabs.md) 的 tab 标记指的是同一个东西，不另造映射。

被压缩过的消息带 `summary` 标记，界面上会打一个小徽章 —— 摘要是总管对一段对话的转述，不是谁真的说过的话，读的人应该能一眼分开。

### 用户是 id 0

界面要能发言，就需要「用户」在消息模型里有身份。用的是 **id 0**：`Surface.id` 的文档明说它永远不为零（`src/Surface.zig:57-62`），所以零不会和任何终端相撞。建群时用户自动入群、且带全部历史。

### 轮询而不是推送

视图每秒拉一次快照（`ghostty_app_chat`），而不是让核心推给它。理由是这个窗口是人偶尔打开看一眼的东西，为它跨 C 边界搭一条推送通道，是为一个「再问一次就行」的视图付一大笔机制成本。

一次调用返回**全部群与全部消息**，所以窗口不会显示出半更新的状态。

### 发送失败不清空输入框

`ghostty_app_chat_post` 返回 false 时输入框内容保留。把人写的东西悄悄吞掉，比让他再按一次发送糟糕得多。
