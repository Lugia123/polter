...
- ~~**R8** —— Poltergeist 不管理任务。界面里不显示任务列表、不提供任务编辑入口。~~ **已改**：右栏顶部有第二个 tab（群聊 / 任务），一行一个任务：状态、标题、负责的终端。**只读**——建、派、关、取消都是总管的工具，界面里没有编辑入口，这一半没有变。见 [tasks.md](tasks.md)。
...
4. **用户能发言** —— 结论见下文专节。
5. **观察窗，不是控制台** —— ~~除发言外只读~~：不在这里按住终端、不在这里停掉监控、不在这里给任何终端敲命令。**已改一处**：`d` 可以把选中的群从**列表**里移除（见「群列表：顺序与 `d`」）。这一条的意思没有变——它管的是「不在这里指挥任何终端」，而收拾自己的列表不是指挥谁。
...
| 几点发言看不出来 | 日志用单调时钟，没有墙钟基准 | **不自动解决**，同上，数据层（现已改为发消息时直接读墙钟，见下） |
...
**顺序：活跃的排上方，其余按最后说话时间倒序，同名次按名字。**

「活跃」= **群成员名单和此刻真实开着的终端有交集**（`Chat.isActive`）。
不新增字段、不需要任何人维护标志。

**是交集，不是名单本身。** 名单上的成员是某一刻有人写下的一个 id；
真实开着的终端是宿主此刻还持有的那个。两者只有在「每一条销毁终端的路径都记得退群」
的前提下才一致——而一个只在别处没 bug 时才成立的判据，等于把自己的正确性外包了出去。
退群这一步确实有（`Surface.deinit` 调 `Chat.forget`），交集照样做：
`isActive` 在那一步万一没跑到的情况下仍然答对。

交集是 O(开着的终端数)，个位数，每个一次哈希查找。

屏幕前的人不需要任何特例：`Chat.user_id` 是 0，而 `Surface.id` 明文规定永不为 0，
所以他从不出现在「开着的终端」那一侧，只有他一个人的群就是非活跃——它本来就是。
「开着的终端」这一侧由 `App.liveTerminals` 提供：`poltergeist/Chat.zig` 是纯模型，
没有窗口、没有时钟、没有磁盘，把 App 塞进去就没有这条了。

这条和「群壳恢复、成员不恢复」天然咬合：**重启后所有群都是非活跃的，
直到总管把成员加回去——而那正是「唤起这个群」的确切含义。**

列表**永不过期**。用户不确定什么时候会唤起某个旧群，所以没有任何东西决定丢掉一个群；
它从 `<state>/chat/<群>/` 这些已有的记录目录推导，那些目录活多久群就活多久。

**但内存里同时只能有 `Chat.max_groups = 32` 个群**，而磁盘上的目录不断累积。
所以启动时按最后说话时间倒序放回，并**留出 8 个空位**（`App.restore_reserve`）
——否则历史长一点的机器一开机就把 32 个格子占满，当天第一次 `group_create`
会被拒绝，而屏幕上没有任何东西解释为什么。放不回来的那些**记录一个字没少**，
少的只是一条被重新选中的路。这条张力还没有一个漂亮的答案（提高上限？列表改成懒加载？），
写在这里是为了不让它被发现两次。

#### `d` 移除选中的群，先弹一个画在 TUI 里的确认框

**裸键 `d`，和现有的 `q`、`t` 一致，不是组合键。** 先要的是组合键，查过之后否掉，
理由留在这里以免重开：

- **Cmd+退格在 macOS 上已被默认键位占用**（`src/config/Config.zig` 绑成
  `text:\x15`，删到行首，是标准行编辑习惯）。占了它就会弄坏一个大家不假思索在用的键。
- **这个 TUI 没开 Kitty 键盘协议**，而传统终端编码里根本没有 Cmd / Super 修饰位
  —— 组合键传不进来。
- 开 Kitty 协议要发 `CSI > 1 u` 并在退出时恢复，**被强关就恢复不了**，
  终端会留在一个用户没选过的键盘模式里。为一个删除键背这个风险不划算。
- **确认框本身就在防误触**，组合键防的是同一件事。**两样只需要一样。**

确认框**画在 TUI 里**，不是系统弹窗：这个界面是终端窗口是有意的，
一个从操作系统弹出来的框会挡在用户正在用的别的 app 前面，就为了问这一个列表的事。

**文案是承重的。** 用户看到「删除」会以为昨晚的对话没了，而它没有：
...
**删的是列表，不是记录。** 这和 [gaps.md](gaps.md) 红线里「记录什么都不删」是同一条，
也是 `group_compact` 当初的同一立场（「它不是删除，摘要写在消息之后而不是覆盖掉」）。
落盘上只是 `group.json` 里一个 `listed: false`，没有任何一个日文件被碰过。

**权限不是问题。** 这里一度写着「界面用的是它所在那个终端的身份，
所以非总管终端里按 `d` 会被拒」——**那是错的**：聊天界面发出的调用身份是
**屏幕前的人**（`App.zig` 里 `if (self.isChatSurface(id)) break :caller .{ .terminal = Chat.user_id }`），
而群主为 0 的群 `rpc.ownsGroup` 认作「没有哪个总管的」，谁都可以收拾。
`d` 本来就能用。

**活跃的群不删。** 判据就是上面那一条，没有第二条：
群里还有开着的终端，`group_destroy` 就返回 `GroupActive` 而不做任何事。
理由是「删它会不会影响到人」——空群没有人可打扰；有终端在里面的群，
删掉会把它们从一个正在工作的对话里踢出去，而且这个群的任务面板跟着一起没
（`App.chatDestroy` 会 `tasks.forgetGroup`）。想删就先 `group_remove` 把人请出去。

这一条**判在宿主，不判在界面**：这样 agent 直接调 `group_destroy` 一样被拦，
界面不需要自己知道谁活着。所以流程是「按 `d` → 确认框 → `y` → 底部红条说明拒绝」，
拒绝的原文是：

```text
that group still has terminals in it. Take them out with group_remove first
-- destroying it would drop them from a conversation without telling them.
```

**副作用要知道**：`group_create` 会把创建它的那个终端放进群里
（`Chat.create` 里 `group.members.put(self.alloc, by, .{})`），
所以**总管自己还开着的时候，它建的群一直是活跃的、删不掉**。
重启之后群壳恢复、成员不恢复，那些群就是非活跃的、能删——
「昨天关掉的 worker 让旧群永远显示有成员」正是被这条修掉的那个症状。
...
**五条后来变成四条，而掉队的那条正好落进上面那句话里。** `76fa175ba` 把「按住本终端」（`Keep This Terminal Working`）从三处菜单一并撤了，命令面板那条随后也去了，于是它今天只剩绑键这一条路——也就是本节写下来警告的那个状态。闸还接着（`clock_out` 仍拒绝被按住的终端），撤的是入口不是机制；这段留着，是因为它是判断要不要把入口放回去的依据。
...
| --- | --- | --- |
| 群列表 | `group_list` 有 | 不变 |
| 消息 | `group_read` 有，时间戳是单调时钟 | 带 `at_ms` 墙钟毫秒（`src/poltergeist/wire.zig:617-618`），发出时即从真实时钟取 |
...
墙钟时间的做法：**消息在发出的那一刻直接读真实时钟**（`daylog.nowMs`），不做任何换算。

早先的做法是「日志继续用单调时钟，启动时把两个钟配一次对，之后换算回墙钟」，那是错的，**错在 `.awake` 这个时钟在机器挂起时是停的**：配对之后每睡一次，换算出来的墙钟就往回退一次，只增不减。真机上表现为——合盖两小时再唤醒，之后发的消息被盖成两小时前，`dayOf` 因此把它写进了前一天的日文件，聊天窗口也画在错误的位置；而进程刚启动时漂移为零，所以它看起来一直是对的。**单调时钟只回答「距上次多久」，没有办法让它回答「现在几点」**，静止时长那一侧仍然（而且必须）用它。
...
5. **重启之后，翻不出任何历史。**`seamOf` 在消息为空时返回 `wait`，`fetchHistory` 就直接 return。理由写在注释里：「空群等它第一条消息到了自然就有接缝」——**这在一次运行内成立，跨重启不成立**。重启后 `Chat.restoreShell` 只恢复群名、简介和时间戳，内存里一条消息都没有，而磁盘上整段日志还在；接缝只能从实时消息来，没有人发言就永远等不到。用户打开昨晚的群，看到一张白屏，记录就躺在旁边的 `.jsonl` 里。修法是一行：空列表返回 `.before = 0`，因为 `before_seq == 0` 在 `ChatLog.history` 里本来就是「无下界，给最新的」，不是这里新造的哨兵值。

第 5 条和第 3 条是同一类，但更隐蔽：它连「跑起来」都不够，**得跑起来、关掉、再跑起来**才会露馅。原来那条测试断言的正是 `wait`，所以它一直是绿的——测试忠实地记录了一个错的判断。
...
- ~~**不显示任务列表**（R8）。~~ **已改**：右栏第二个 tab 就是任务面板，只读。见 [tasks.md](tasks.md)。
- **不做通知气泡**：未读走 tab 标记，归 [tabs.md](tabs.md)。
- **`d` 不删记录**，只把群从列表里拿掉，而且**活跃的群拿不掉**。见上面「群列表：顺序与 `d`」。
...
- 界面里**不编辑任务**。显示是有的（右栏第二个 tab），编辑没有：建、派、关、取消都是总管的工具。见 [tasks.md](tasks.md)。
