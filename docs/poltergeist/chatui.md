...
> 状态：**推倒重来中**。macOS 原生窗口做出来过，用过之后被否掉了，代码已删除（见「决策变更」）。现在回到本章原本推荐的候选一：终端内 TUI。
...
参照的是 Claude Code CLI 自己的样子 —— 一条消息流，加一个输入框，别的都不要：
...
│  看看进度                                       │
│                                                │
│  ✳ Write retry.py decorator             01:24  │
│  retry.py 已就绪，签名如下：                     │
│    def retry(attempts=3, base_delay=0.1, ...)  │
│                                                │
...
- **顶栏**：当前群名 + 人数。人数就是「群里有几个人」那条诉求；按一个键展开成员清单。
- **作者行**：显示**终端标题**，也就是 tab 上那个标题的同一个字符串。这是「和 tab 对得上」的做法 —— 不是把 id 印得更好看，而是根本不印 id。
- **时间**：右对齐的墙钟时刻。
- **正文**：终端里天然等宽，代码片段自然对齐；agent 之间贴的日志片段带 ANSI 颜色也能照原样显示。
- **底部**：输入框，回车发送。

群之间用 Tab 键切换，或者数字键直达。

### 身份：宿主注入一个用户令牌

这是唯一的新机制，也是唯一需要想清楚的地方。

TUI 是一个独立进程，要通过已有的 unix socket 读写群聊。但现有的令牌认证的是**终端**（`Surface.id`），而聊天界面要以**键盘前的人**（`Chat.user_id`，也就是 0）的身份说话 —— 用户是每个群的成员，发言显示为 `you`。

**做法**：Polter 在为聊天界面创建 surface 时，额外注入 `GHOSTTY_POLTER_USER_TOKEN`。服务端把这个令牌映射到 `user_id`。

- 只有 Polter 自己开的聊天终端拿得到它，被监督的 agent 终端**拿不到** —— 它们的环境里只有自己那份终端令牌。
- 身份仍然由宿主注入而非进程自称，与 [mcp.md](mcp.md)「不接受客户端自报家门」的原则一致。
- 用户在聊天终端里跑别的程序当然也能读到这个变量。那是用户自己的终端，用户自己负责；关键是这条路径不会让**agent**取得用户身份。

拒绝过的替代方案：让任何持有有效终端令牌的客户端都能「声明自己是 UI」。那等于任何被监督的 agent 都能以用户身份发言，其它 agent 会把它当成人的指示 —— 这是权限提升，不是便利。
...
- **不做搜索、不做历史归档、不做富文本渲染。**这是观察窗，不是聊天软件。
...
Poltergeist 的聊天日志拟落在 `${XDG_STATE_HOME}/polter/chat/`。**尚未实现** —— 目前聊天记录只在内存里，进程退出即消失。
