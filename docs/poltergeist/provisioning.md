# 注册：让各家 agent CLI 知道 Polter 在这儿
...
**但工具不会自己出现**——MCP 客户端只加载它被配置过的服务器。注册插件就是把
...
前面两样（MCP 注册、skill 镜像）都是让工具**够得着**。这一样是让一句话**被读到**，
它解决的是另一个问题。

**问题**：skill 是按「用户要求了什么」去匹配的，而没有任何东西匹配得上
「你刚把一件事做完」。所以 worker 做完活，把交接写在自己屏幕上，然后停下——
**而那块屏幕上的字不到任何人手里**。主管看到的只有「屏幕不动了」，
它分不清这是做完了还是死了，只能自己去 `terminal_read`。

**证据**：真实记录里 71 个任务只产生了 15 次 `done`、1 次 `blocked`。
写在 `operating-a-terminal` 里的那条规则，被这个数字证伪了——它当然还该在那儿，
但它一个人不够。

**做法**：往各家 CLI **每一轮都会读**的那个规则文件里，写一个带标记的块。

```sh
host_rules_file() { printf '%s/.claude/CLAUDE.md' "$home"; }   # 空 = 这家不写
```

四条约束，每条都有理由：

1. **只写用户级，绝不碰项目级。** 项目里那个 `CLAUDE.md` 属于仓库的主人，
   而「Polter 装在这台机器上」不是关于他那个仓库的事实。
2. **带标记，原地替换，可以收回。** `<!-- BEGIN POLTER (managed …) -->` 到
   `<!-- END POLTER -->`。标记外面的一个字节都不动，参数设 `no` 就把这一块拿走。
3. **内容只说机制，不抄规则。** 「你打印出来的东西不到任何人手里」是关于
   这套东西怎么运作的事实，别处的指令再多也推不出它；规则本身留在 skill 里，
   那儿爱写多长写多长，不占每一轮的上下文。规则文件是这台机器上最贵的散文。
4. **不知道就不写。** 七家里只有三家（Claude Code / Codex / Gemini）填了路径，
   其余四家 `host_rules_file` 返回空。**往别人 home 目录里写错文件，比不写更糟。**

幂等和 skill 那边一样是「先读后写」：块和要写的一模一样就什么都不做，
不然一次启动重写一遍用户手写的文件，就是在等那一次把人家笔记吃掉的竞态。

> 两半都在真机上驱动过：`sh` 那份和 `pwsh` 那份各跑了同样五种情形（新建 /
> 幂等 / 用户在上下写了东西时替换 / 关掉后收回 / 宿主不支持）。
> PowerShell 那份**因此抓到一个 bug**：空数组会被 unwrap 成 `$null`，
> StrictMode 下 `$null.Count` 直接报错——只读代码是读不出来的。
...
所以配一条：**注册插件在启动时看一眼各家二进制在不在 PATH 上，发现了就通过通知
...
1. ~~**七家一家都没有真机验过。**~~ **2026-09-01：六家验过了**，判据是**那家 CLI
   ...
   | `codex` | `codex mcp get` 逐格回显；**`env` 的键回显、值被打码**（那是它的对：env 里通常放密钥），值由它自己写的 TOML 确认 |
   ...
   **验证方式记在这里，因为它可复用**：先确认那家 CLI **遵循 `$HOME`**（**四家单独验过**：
   `gemini` / `opencode` / `claude-code` / `codex`，**没有一家是假设的**）。
   **`qwen-code` 是例外，它的 `$HOME` 行为从来没验过**——Windows 那次走的是另一条路：
   用真 HOME，靠「先拷贝、逐字节比对、比不上就中止」保安全。**那条路有效，但它不是
   关于 qwen 怎么处理 `$HOME` 的证据**，需要沙箱时不能照搬，在假 HOME 里跑出厂脚本，再用那家自己的
