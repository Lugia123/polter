...
Polter 自己一直在写两份：`$XDG_STATE_HOME/polter/chat/chat.jsonl`（流）和
`$XDG_STATE_HOME/polter/chat/<群>/<日期>.jsonl`（记录，给人和 AI 读的）。两份
都不依赖任何插件，**而且这个插件也不读它们**。所以这个插件的价值只在**送到别处
去** —— 数据库、共享盘、公司内网。

消息先同步写进核心自己的存储，写完才算数；然后宿主把它作为一个**实时事件**交给
订阅者。数据库连不上、凭证过期、psql 被卸了，都只是「不确认」而已：没确认的事件
留在订阅队列里，下一轮原样再来一次。**代价说清楚**：这个插件停久了会丢它自己那
一份（队列有界，满了丢最老的并计数），核心的记录一条不缺。
