...
写 `$XDG_STATE_HOME/polter/archive/<file_name>`，一行一条：

```json
{"stream":"default","seq":1043,"at_ms":1786819271275,"group":"build","author":"worker-core","summary":false,"text":"…"}
```

目录由插件自己拼，`file_name` 只贡献最后一段。这不是洁癖：`file_name` 是工具面
能写的参数，如果它能表达路径，一条从"配置一个存档插件"到"把聊天正文持续追加进
`~/.ssh/authorized_keys`"的路就通了。
...
坏 `schema` / 坏 `stream` / 带 `..` 的 `file_name` 各喂一次，断言都是 exit 2、
stderr 只有一句、句子里不含喂进去的值，且那条 `../` 没有在目录外留下文件。
光断言正则本身是不够的 —— 把 `file_name` 那个判断删掉，别的用例全都照样绿，而
聊天正文开始写到插件选的目录外面去。
