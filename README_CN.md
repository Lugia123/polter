...
**有两个词不是一回事**，后面整篇都依赖这个区别：

- **Polter** 是那个终端，就是这个程序。它只测一个量（屏幕多久没动了）并且传话，
  **它从不做任何判断**。屏幕不动到底是"卡住了"还是"在想"，它拒绝回答，因为答错
  比不答更糟。
- **总管**是你派去管事的那个 agent。读屏幕、下判断的是它。它就是一个普通的
  Claude Code 会话，你随时可以看它在干什么，也随时可以把键盘抢回来。
...
总管跟别的会话一样是要烧 token 的 —— 它就是一个 Claude Code 会话，整夜读屏幕、
写消息。给它配一个你愿意让它一直跑着的模型和预算。
...
| **Linux** | 没有二进制包。GTK 版编得过，但没有人在上面真跑过一次监管，而发一个谁都没启动过的包不该悄悄地做。要用请从源码构建。 |
...
skill 装出去的供给插件要在 `PATH` 上找你的 agent CLI —— 从终端起它找不到，然后
**安静地退出**，什么都不说。

### Windows：能用什么，不能用什么

解压到任意目录，运行 `polter-host.exe`。**两个 DLL 和 `share/` 必须留在它旁边**
—— DLL 是启动时按名字加载的，`share/` 里放着总管要读的 skill 和供给插件。没有
...
| | |
| --- | --- |
| 在一台 Windows 11 上验过 | 窗口能开、标签页能用、shell 能起、含中日韩文字渲染正常、输入法能打出汉字、菜单和快捷键可用、资源目录找得到、供给插件能启动。 |
...
每个工具做什么、拒绝什么，全在
**[`docs/tools_CN.md`](docs/tools_CN.md)** 里 —— 它是查阅材料，读起来也像查阅材料。
**你不需要它就能用 Polter**：总管会自己读它的 skill 然后自己调这些工具，下面那个
[完整例子](#完整例子)才是你实际操作的样子。
