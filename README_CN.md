...
<p align="center">
  ...
  <b>Polter 自己不要账号、不要 API key、不发一个网络请求</b>；但被它盯着的那些
  agent 是你自己的 CLI，它们照旧跟自己的模型说话，花的还是你本来就在付的那份钱。<br>
...
| **往任何一个 tab 里打字** | 它能把卡住的 worker 弄动，或者叫它换个法子。**但它按不了那个"允许"** —— 权限确认只会来叫醒*你*，几点都一样。 |
...
**三、它是个实验，端到端只在 Claude Code 上跑通过。** 底下是普通的 MCP（你的
agent CLI 本来就用它够工具）加普通终端，所以别的 CLI *应该*也能用——但除了它，
...
就都归它管了。Polter 什么都不调。它只是在一个 tab 里把你指名的那个 CLI 起起来，
然后读屏幕。那个 CLI 怎么登录，是它跟它厂商之间的事。于是：

- **你的订阅算数。** Claude Code 已经用你的套餐登着，那让它当总管就不会多出一分钱
