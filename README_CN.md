...
- **关于这一夜的一页算术。** 任务活了多久、谁在说话说了多少、哪些终端静止了多久，
  按小时排开。它只做算术，不下结论 —— 超过你设的线的任务标成 `over`，那只表示
  「过了你设的那条线」，不表示出了问题。
...
| 没人试过的 | 群聊界面在 Windows 上没有人真的用它聊过。那个界面和 macOS 是同一份共享代码，菜单也能打开它，所以按理能用 —— 但「按理」不等于「见过」，所以放在这一行。 |
