...
| [stats.md](stats.md)          | —          | 统计页（第三个 tab）：程序摆事实、总管做判断；八格布局；不做定时强制总结 | 判据是「这句话程序说得了吗」；真实数据否掉了按自然日切和按 `progress` 分段两种画法 |
...
`dev-docs/agent-loop-design.md` 是本设计的第一版草案，已被本目录取代并**删除**（该文件未被 git 跟踪）。仍然成立的内容已吸收进本目录，被推翻的部分逐条记录如下，避免日后有人凭记忆把它们改回去。
...
`src/poltergeist/main.zig` 的 `test {}` 块里必须逐个 `_ = Module;`。**Zig 只分析被引用到的东西**——在 `pub const` 里导出一个模块不算引用它的测试。一个既不在 `test {}` 里、又没有任何模块 import 的文件，它的测试既不会失败也不会运行，而且没有任何东西会提示你。

**「引用」不等于「写进 `test {}`」，这一点实测澄清过。** `daylog.zig` 在 `main.zig` 里一个字都不提时，它的 7 条测试照样被收集了——因为 `ChatLog.zig` import 了它，**传递可达的模块的测试是会跑的**。所以 `test {}` 那行是穷举清单约定，是唯一不依赖「碰巧有人 import 了它」的保证；但**不要拿测试数变化去证明它生效**，涨了可能只说明这个模块被别人 import 到了。
