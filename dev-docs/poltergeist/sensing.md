...
**4. 事件之外还有一条不是事件的通道：`Sampler.heartbeat`。**

本章的推导是「只在转换时上报」，而这条推导漏掉了下游：`Bus.quietMs` 存的不是时长，是**上一次被告知的时长加上从那以后过去的时间**。这个外推只有在「屏幕一动就会有人说话」时才成立，而只在转换时上报恰恰不满足它——一个静止过一次、之后一直在干活的终端，`markChanged` 一条 `resumed` 之后就再也不出声，那个数就从 0 一路涨上去。实测报出过 29 和 31 分钟，而两块屏幕都在动（见 [gaps.md](gaps.md)）。

所以屏幕在动时每 `heartbeat_ms`（默认 5 秒）把当前的 `now - last_change_ms` 说一次。**它不是事件**：不进 `Event`、不产生 notice、不计 rounds、不写日志，`Bus.noteQuiet` 只拿它更新那两个用于外推的字段。已上报静止期间它沉默，因为那段时间的外推本来就精确。
