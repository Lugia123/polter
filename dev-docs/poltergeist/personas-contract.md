...
1. `tools/list` 不再纯本地。先问 host 一次
   ...
   **`persona_face` 不带终端 id，只问调用者自己。** 当时的理由（`41ba1d068`
   的提交说明，和 `rpc.zig` 里 `persona_face` 的注释）：
   ~~身份来自握手时证明过的 token，所以根本写不出「*那个*终端被允许做什么」
   这个问题——带这个参数的能力面，同时也是一个踩点面。~~
   **2026-09-22 用户改了这一条的适用范围：总管必须能查任意终端（worker，也包括
   别的总管）的能力边界，否则没法分配任务。** 于是新增了仅总管可用的
   `terminal_capabilities(id)`（roles.md 第十三节）；`persona_face` 本身**不变**，
   仍然不带 id、仍然只问自己。
   划掉而不是删掉，因为这个理由对 worker 仍然成立，只是对总管不成立：
   踩点担心的是**低权限方摸别人的边界**，那条路（worker 查别人）继续关着；而
   总管本来就能 `terminal_read` / `terminal_send` 任何没有护盾的终端——读屏幕、
   直接问那个 agent——它的可及范围已经覆盖这些信息，结构化查询**不新增可及
   范围**，只是让它不用猜。护盾终端照旧拒绝（`Shielded`）。
