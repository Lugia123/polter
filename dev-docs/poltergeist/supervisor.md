...
- **R6** —— 「总管执行完 skill 后要不要让该终端下班」，以及用户怎么表达「这个不许停」。落在「按住」。
...
托管流程：用户开群聊 → 把第一个终端指定为总管 → 让它监督另外三个 → 把不许停的那个按住（研究终端）→ 人去开会。
...
理由：R5 的用户同时处理开发与研究两摊，两摊的判断标准不同，硬塞给一个总管等于逼它在两个领域之间来回切上下文。

禁止总管链是因为上班/下班是**终端级**的：一旦允许递归，「让总管的总管下班」在语义上没有定义。

推翻条件：如果出现单摊超过十来个终端、确实需要分组的真实需求，再引入一层层级，而不是现在预留一个用不上的图结构。

#### 「至多属于一个总管」现在只管通知，不再管可达性

这一条曾经被写成「星形拓扑」：总管够得着自己名下的每一个终端，被监督终端谁也够不着，而单归属**从结构上排除了两个总管同时对同一终端下指令的竞态**。

**那部分被推翻了，是有意的。** 代价太具体：两个总管永远无法互相打断、互相重启，而「互相 Ctrl+C 再把对方拉起来」正是重载插件这类只有重启才生效的东西唯一的做法。同样被卡住的还有 `group_add`——它按终端 id 判权，而「谁在管这个总管」对总管恒为空，所以**一个总管永远拉不进另一个总管**，两个总管连话都说不上。

现在的规则**只看被干涉的那一方**：总管可以干涉任何 Polter 终端；非总管碰不到任何带标记的终端（`supervisor` 或 `watched`），但可以碰没有标记的。完整的表、错误码分工和四个验收场景在 [mcp.md](mcp.md)「可达性：只看被干涉的那一方」。

单归属本身留着，含义收窄成一句：**通知送给谁。** 一个终端只有一个 minder，所以它静止的报告落进一个箱子，而不是被复制进好几个。

「两个总管同时对同一终端下指令」这个竞态**不再由结构排除**，改由别的东西处理：终端的 `held`（用户按住，谁也不许让它下班）、`shielded`（用户把它挪出工具面，对谁都拒，总管也不例外），以及总管自己在群里说清楚谁在管什么。这是一次明确的交换——用一个被排除的竞态，换回两个总管能互相操作。
...
- **off-duty（下班）** —— 总管判定无需继续工作后打的标记。被按住的终端进不了这一态。
...
quiescent      -> off-duty    总管结论 = 无需继续
                              （rounds 由程序供数，够不够是总管判断）
                              且 未被用户按住                           总管
off-duty       -> on-duty     手动复工                                  仅用户
任意态         -> 关系删除     surface 关闭                              程序
--------------------------------------------------------------------------------
终端被按住时，指向 off-duty 的那条边在 MCP 入口被直接拒绝（`TerminalHeld`）。
反过来不会发生：`clockOff` 拒绝按住的终端，所以「又被按住又下班」这个组合
到不了——tab 标记里因此只有「按住+在动」和「按住+静止」两种，没有第三种。
```
...
复位条件两条：提交了「继续」、用户在该终端手动输入过。（曾经还有第三条「模式被改」，随工作模式一起没了。）

**那个 n 现在没有配置面，程序只报数。** 曾经的打算是 skill frontmatter 里一个
`max_rounds`；它已经删了，理由是**没有任何代码读它**——被解析、被校验、然后扔掉，
而用户写下它会以为配置了什么。所以现在是：`Entry.rounds` 随 `terminal_list` 给
...
**一个布尔，只有用户能设：`Entry.held`。** 立起来的意思是「这个终端不许停」。

它取代了三种工作模式（下班 / 定向无限 / 接续无限）。为什么整套换掉，在
[README.md](README.md) 里 P5、P6 两条原则的记录中；这里只说它现在的形状。

**曾经的两个「无限」子型（定向、接续）没有留下任何痕迹，这是对的。** 它们对
Poltergeist 的差别只有一点——**任务从哪来**——而 R8 已经规定 Poltergeist 不管理
任务。所以它们在程序侧的行为本来就完全一致（都禁止下班），差别只体现在总管读
哪份 skill。一个不产生任何程序行为差异的区分，删掉之后什么都没少。

### 为什么只有用户能设

`Bus.setHeld(id, held, who)` 的第三个参数是 `Authority`，`who != .user` 一律返回
`NotPermitted`。**这比旧规则还硬**：旧规则下总管至少还能在两个无限模式之间挪。

理由是：**按住是用户的一句话，不是运行时状态。** 而总管每次被唤醒本来就在做
「还有没有必要继续」这个判断——如果它还能改这句话，那它就同时是被约束的一方和
定约束的一方，约束不成其为约束。

这条要写进 `supervising` skill，否则总管会去调、拿到 `NotPermitted`、然后开始猜
自己是不是哪里做错了。

### 统一抽象

一句话：**总管执行完 skill 后，决定要不要通过 MCP 让该终端下班；被用户按住的
终端，这个动作由程序拒绝。**

- 未被按住 → 允许；`rounds` 够不够由总管自己判断，程序只供数。
- 被按住 → 程序直接拒绝，返回 `TerminalHeld`。
...
推翻条件：若实践中确实需要「用户在场时临时豁免」，正确做法是**用户**自己把按住松开，而不是给 AI 开一个后门。（这一项今天既不在菜单也不在命令面板里，唯一的入口是自己给 `poltergeist_toggle_held` 绑一个键。）
...
R3 给的是二选一：(a) 总管代理用户决策；(b) 通知用户。本设计让这个选择**按被监督终端各设**，而不是全局一个开关——理由与按住是按终端设的同一条：同一个用户的开发终端和研究终端，能容忍的自作主张程度本来就不一样。
...
**明确记录被删除的旧设计**：旧稿里那个「一次性冻结全部」的快捷键（原名 `agent_stop_all`）、与它配套的全局启停配置项、以及「默认全关、按 surface 显式启用」这三项，在本次修订中删除。前两项建立在「能冻结一切」的错觉上；第三项与 R5 的实际流程冲突——用户是先派完活再托管，不是先逐个打开开关。这三项的原始出处 `dev-docs/agent-loop-design.md` 已被删除，只能沿用 [README.md](README.md) 对它的记录。
...
**第二步现在是真的由程序做的，而且只做到群壳为止**（`src/poltergeist/GroupLog.zig`）。
群名和简介从 `<state>/chat/<群>/` 推导回来，**成员一个都不放回去**——上面第 2 条
理由（「这个终端是不是上次那个」是判断）只对成员成立，群名是一个字符串。
所以重启后每个群都是空的、非活跃的，而**把终端加回去正是第三步**。

「非活跃」在这里是有定义的：**群成员名单和此刻真实开着的终端没有交集**
（`Chat.isActive`，见 [chatui.md](chatui.md)「群列表」）。不是「名单为空」——
名单会腐烂，交集不会。终端关掉时 `Surface.deinit` 会把它从每一个群里摘掉
（`Chat.forget`），交集这一步照做，为的是这条判据不依赖那一步没出过 bug。

**活跃的群不能销毁**：`group_destroy` 对还有开着的终端的群返回 `GroupActive`，
先把人 `group_remove` 出去才谈得上删。这一条也解释了第三步的一个直觉——
你正在用的那个群，不会被谁顺手从列表里拿掉。
...
| 每个成员上次的角色 | 重新认领之后照着设回去 |
| 每个成员上次**有没有被按住** | 记下来，但总管**设不回去**——见下 |
...
`Session.Member.held` 是**存的**（`Session.zig:166` 写、`:251` 读回，两条测试
坐实）。但存下来的用处和别的字段不一样：

- 角色、工作目录、标题 —— 总管读回来之后可以**自己设回去**
- 按住 —— 总管读回来之后**只能看**。`setHeld` 对 `who != .user` 一律
  `NotPermitted`，总管调不动

**这不是漏了一个恢复动作，是刻意的。** 按住是用户说过的一句话；重启之后由程序
替他再说一遍，那句话就不再是他说的了。所以快照的作用是让总管**知道**「昨晚这台
是被按住的」，从而在报告里提一句、让用户自己决定要不要再按上，而不是替他按。

（这也是为什么它出现在上面那张表里而不在下面「瞬时状态」那一节：它不是瞬时的，
它是用户的意思。）

**瞬时状态不还原，也不该还原。**

把 `rounds` 存下来读回，等于宣称「昨晚数到 3 次，今天接着数第 4 次」，可昨晚那个进程早就没了，被监督的 agent 也重启了。**这个计数跨重启没有意义**，硬要延续只会让它变成一个说不清的数。静止时长同理：它是关于「现在」的度量，重启之后从零开始才是诚实的。
...
### 能承诺放回的只有两样：目录与 tab 名

**终端里跑的不一定是 AI。** 可能是一个开着构建的 shell、一个编辑器、一个 `tail -f`。「把上次的会话恢复回来」在这三者里意思各不相同，在其中一些里根本没有意义。

所以落盘记的是每个开着的终端的 `cwd` 与 `title` —— **包括不在任何群里的那些**，因为它们在别处没有任何记录，不记就等于重启后凭空消失。`terminal_list` 现在也对屏幕上每个终端报同样这两项，不论有没有人在看它。**这两项就是匹配用的全部依据。**

确知跑的是 agent 时，除了终端本身，它自己的会话可能也值得要回来 —— `claude -r` 配上笔记里的目录通常就是这个意思。但那是**在你有理由相信它适用时可以建议的加分项，不是「恢复一个终端」的定义**。分清这两者要紧：把 `claude -r` 当成契约，第一个普通 shell 就会把它证伪。
