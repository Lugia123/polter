...
### 5.2.9 渲染线程停在哪一句：`-Dlog-render-phase`

`[rthread]` 能说「渲染线程卡在自己的回调里」，**说不出卡在哪一句**——
`drawFrame` 里八个等待点在它眼里长得一模一样。`[rphase]` 就是为了把它们分开。

```
zig build -Dtarget=x86_64-windows-gnu -Doptimize=ReleaseSmall \
  -Dlog-render-phase=true -Demit-macos-app=false -Demit-xcframework=false
```

**默认关。** 打开后每次唤醒约两行，一格持续输出约 14000 行/小时——足以淹掉
`[rsz]` / `[blit]` / `[mbox]`，所以它只出现在专门查这件事的包里。

**⚠️ 为什么不给它加额度**：一个长时间安静之后才冻住的 pane，额度早在启动时
就花光了，「一行没打」和「根本没走到那句」于是长得一样——而那正是这行要
分开的两件事。宁可用开关，不用额度。
...
| `at=` | 守的那一句 | 读法 |
|---|---|---|
| `zerosize` | 尺寸为零的早退之前 | 打了却没有后续 ⇒ 尺寸真的是零 |
| `noredraw` | 「不需要重画」早退 | **带四个分量**，直接说是哪一项为假 |
| `presentlast` | 早退路的重呈现 | 稳态下绝大多数唤醒走这里 |
| `nextframe` | 等交换链的空闲帧 | |
| `fontlock` | 取字形网格的共享读锁 | 这把锁是**全 app 共享**的 |
| `finish` | 阻塞到 GPU 画完 | |
| `swap` | 交换缓冲 | |
| `health` | 健康上报（阻塞发送） | 停在这里 ⇒ 那一格永久不画 |
| `syncout` | 同步输出模式的早退 | 出货包里**原本一个字都不出**（那行是 `debug` 级） |
| `decide` | 「要不要重画」算完之后 | **无条件打**，带五个量；`presentlast`/`noredraw` 走不走都有它 |

🔴 **⚠️ Windows 上实测：完全空闲的终端每次唤醒都走【完整绘制路】。**
一次唤醒的完整块是 `zerosize → nextframe → finish → swap`，
`presentlast` 和 `noredraw` **一条都没有**——`needs_redraw` 次次为真，早退路
从没走过。⇒ **这两个 `at=` 的缺席是常态，谁都不要拿它们的缺席下结论。**
`health` 和 `syncout` 同样是零条，但那两条**是否可达尚未验证**，与上面两条
不是一回事。

⚠️ **`decide` 这一行是补上来的，起因是一个设计错误**：那五个量原先打在
`noredraw` 那条早退分支里，而那条分支在真机上一次都没走过 ⇒ **恰好在
`needs_redraw` 恒真、最需要知道「凭什么真」的时候，一个量都读不到。**
**仪器挂在分支的一侧，就是在赌它会走那一侧；判断量要在判断【之后】无条件打。**

**⚠️ `at=swap` 那一行的 `r=` 和别人不是一个命名空间**：它打的是图形 API 对象的
地址（和 `[blit]` 同源），其余 `[rphase]` 和 `[rsz]` 打的是渲染器对象的地址。
⚠️ **两者的差【不是常数】**——先前这里写的是「差一个固定偏移」，实测两次分别
是 `0x1260` 和 `0x1AC8`。**按 `at=` 这个标签认它属于谁，不要按 `r=` 的数值算。**

**开关同时改一件事**：`[rthread]` 心跳平时每 8 次唤醒才打一行，开关打开后每次
唤醒都打。⭐ **这提升的是「卡住的那一次也会留下最后一行」，不是让下面那个判据
变得可用。**

🔴 **⚠️ 唯一可用的判据是「`wakeup` 这个数还在不在涨」，开关开关都一样。**
先前这里写的是「开关打开后『最后一行的 `wakeup` 比 `completed` 大 1』这个判据
才成立」——**那句是错的，已删。** 心跳打在干活【之前】，所以一个健康线程在第
`N` 次唤醒时打出的必然就是 `wakeup=N completed=N-1`，干完之后它不再出声，要到
第 `N+1` 次才打。⇒ **那个形状在健康和卡死两种情况下逐字相同，恒真，零分辨力。**
**和采样门控无关，去掉门控也救不回来。**

**成本（实测，x86_64-windows-gnu 的 `ghostty-internal.dll` 字节数）**：
ReleaseSmall `25,574,912` → `25,576,960`（**+2,048**）；Debug `+11,264`。

**⚠️ 两条 `[rsz]` 共用一个额度，而只有 `surface=` 那条消耗它。**
`target=` 那条只是被剩下的额度允许或拒绝。今晚三个人因为这个不对称
各错了一次行数预测。**两条都有「变化时必打」的逃逸，所以稳态下的沉默
是设计意图、不是症状。**
...
2026-09-01 夜补上，独立成篇：`dev-docs/windows/s4.md`。
