...
用远程注入工具的**按键**接口（发真实虚拟键/扫描码的那个，不是直出 Unicode 的
「输入文本」接口），注意两条：
...
> 然后读日志 —— **判据有两半**：那个键的 `[key] … -> surface_key=` 行出现了，
> **并且这一轮里没有 TSF 把键截走的行**。
>
> | 读数 | 处置 |
> | --- | --- |
> | 有 `[key] … -> surface_key=` 行，**且本轮没有 `TSF ate` / `pump swallowed` 行** | **第 0 步通过**，可以进判读表 |
> | 出现 **`[key] TSF ate … composing=Some(true)`** 或 **`[key] pump swallowed … composing=Some(true)`** | **本轮作废，但这不是「日志缺席」——是输入法进了组字态。** 切英文再重来 |
> | **什么都没有** | **本轮作废**，问题在前台或注入工具 |
>
> **不出来、或被 TSF 截走，都不进判读表。**

> ⚠️ **判据的字段是照实现核过的，而第一版写错了，记在这里免得有人改回去。**
> 第一版写的是「`[key]` 行显示 `composing=Some(false)`」。**`keys.rs` 那条
> `[key] msg=… vk=… keycode=… mods=… text=… -> surface_key=…` 里根本没有
> `composing` 字段。** `composing=` 只出现在 `main.rs` 的三条 TSF 相关行里
> （`pump swallowed` / `kept from TSF` / `TSF ate`），**而那三条都是异常路径**：
> 键一切正常时它们一条都不打。
>
> **所以「读到 `composing=Some(false)`」这个判据在正常路径上永远不可能满足**——
> 照它执行，每一轮都会被判成「第 0 步没过」。**判据要写成「那三条不出现」，
> 不能写成「读到 false」。**

**中间那一行是 2026-09-03 补的，而它补上之前，那两种结局长得一模一样**（都是「第 0 步
没过」）——**而处置完全相反**：一个是去切输入法，一个是去查前台。WT 当天正踩在这里：
打 `echo` 两个字母，中文输入法把它组成了「惑臭」，TSF 进入组字态，之后每个和弦都被吃掉。

**它的位置就是它的力量**：它不是在判读表里补一行「如果什么都没有就是前台问题」，
而是**在进表之前就把「缺席」挡在外面** —— 缺席永远不会被解释成某一种成因，
**因为它根本到不了解释它的地方。**

#### 第 0 步和 §2.0.1 不冲突，而说清为什么很重要

§2.0.1 的规矩是「**确认状态的动作，不能是会改变状态的动作**」，并且点名禁止了
「打个字母看看」。**而第 0 步要做的正是打一个字母。** 读起来像一条例外。

**没有例外。** 第 0 步那个动作**确实会改变状态**（它可能把输入法推进组字态）——
它之所以仍然可用，**唯一的原因是它同时报出了自己造成的那个状态**：
`composing=Some(false)` 就是那份读数。

**所以规矩没有松动，只是那个动作自带了读数。** 一个不报 `composing` 的
「打个字母看看」，仍然被 §2.0.1 禁止，而且禁止的理由一个字都没变。

> **这两节是同一天写的，而它们没有互相核对过** —— 一节立规矩、一节写流程，
> 各自都对，合起来打架。**规矩和流程写在不同小节时，谁都不会去读另一处**，
> 而冲突要等到有人照着做才会暴露。这条修正本身就是那次暴露的产物。
...
> **`[key]` 行不是每次按键都打的**，而**它按什么节流，取决于你手上是哪个包**。
> ⚠️ **先判断这一点，别跳过**——两个包对「没有 `[key]` 行」的读法不一样。

##### 怎么判断（只看日志，不看源码）

**在一个已经按过 20 个以上键的会话里，连按同一个普通字母 5 次**，然后数 `[key]` 行：

| 数到几条 | 你手上是 | 「没有 `[key]` 行」怎么读 |
| --- | --- | --- |
| **恰好 1 条** | **新包**（带 `first_of_its_kind`） | 见下面「新包」 |
| **0 条** | **旧包** | 见下面「旧包」 |

⚠️ **别用 `Shift+Home` 做这个判断**——它正是要测的东西，用它判包等于用结论当前提。

##### 新包：每一种键，第一次按下时一定出声

条件是 `if n <= 20 || modded || novel`，`novel` 来自
`first_of_its_kind(msg, vk, ev_mods, consumed_by_core)` ——**拿到的是完整的
`ev_mods`，不是掩过的**。所以每一个不同的 (消息, 键, 修饰键, 核心的回答) 组合
**第一次发生时必定有一行**，重复的才安静。

⇒ **判据变成**：**这个键这次会话里第一次按 ⇒ 必须有行；没有行 = 它没到。**
⚠️ **第二次以后没有行是正常的**，所以做这一格**要用一个你这次会话没按过的键**，
或者先换一个修饰键组合把它变成新组合。
⚠️ 攒够 512 种之后会打一行 `distinct key kinds reported; further new ones are not
tracked`，**那行出现之后缺席又不含信息了**。

##### 旧包：开头 20 个键之后，不带 Ctrl/Alt/Win 的键一律不记

条件是 `if n <= 20 || modded`，而 `modded` 只看 **Ctrl / Alt / Super**——**不看
Shift**。`Enter`、方向键、**`Shift+Home`、`Shift+PageUp` 全都不记**。

真机上验到过：有五分钟一条 `[key]` 都没有，而那段时间 Enter 在执行命令、
`Shift+Home`/`Shift+End` 在搬视口；紧接着的 `Ctrl+Shift+T` 有完整的 `[key]` 行。
**日志路径是活的，只是那些键按设计不记。**

所以在**旧包**上，「没有 `[key]` 行」有**三种**含义，它们看起来一样：
...
**在新包上第一行不成立**：不带修饰键的键第一次按下也会出声，所以「不带
Ctrl/Alt/Win 所以正常」不再是一种解释。
...
远程注入工具后来修了按键注入的扫描码（修之前 `sent` 里 `scan` 全是 0）。如果那次测试用的是
修复前的版本，注进去的 `Ctrl+C` 扫描码是 0，核心解成 `.unidentified`，Ctrl-C 就是不生效
...
1. **用真鼠标点一下被测窗口**（§2.0）。**不要**用 `SetForegroundWindow`，也**不要**用
   「打个字母看看」来确认焦点。
2. **把输入法切到英文**，然后按一个带 Ctrl 的和弦，**确认这一轮里没有 `[key] TSF ate …`
   或 `[key] pump swallowed …` 行**。出现了就停下——那是组字态，后面每一格都会作废（§2.0.1）。
   **不要去找 `composing=Some(false)`：正常路径上根本不打那个字段**，理由见 §2.0.2 的
   那条 ⚠️。
...
**第 3 步不是形式**：`[key]` 行是有节流的，⚠️ **而节流规则取决于包**（见 §2.1，那里有只看日志的判包步骤），
...
| `0x2e` | `0x2e` | `false` | 注入正常但核心拒绝 —— **真缺陷，在核心侧**，与注入工具无关 |
...
> Ctrl/Alt/Win，而 `[key]` 行是有节流的，⚠️ 规则随包不同（见 2.1）。
...
>
> ⚠️ **但 P3 证明的只有「复制这条路是通的」，到此为止。**
> **不要拿它去证明 P1 的匹配路是好的。** 两个键的绑定都写成
> `.{ .key = .{ .unicode = 'c' } }`，都只能靠 `unshifted_codepoint` 匹配上，
> 而那个值来自同一次 `MapVirtualKeyW(VK_C, …)`（status.md §8）。
> **所以那个布局问题如果存在，两个键会一起坏——它们不能互相当对照。**
> P3 绿而 P1 红，成因在选区或 `selection-clear-on-copy` 那一侧，不在匹配上；
> **两格一起红才轮得到怀疑匹配**，而那时 P3 自己也不是地板了。
>
> **P4 —— `Ctrl+V`**
...
| **`keys.rs` 里决定「哪些键会被记进 `[key]` 行」的那个条件**（曾经是 `n <= 20 \|\| modded`，而 `modded` 不看 Shift；今天是 `n <= 20 \|\| modded \|\| novel`） | **本文里每一条以「日志里应该出现 / 不应该出现某行」写成的判据** | **不会，而且比上面几行更难发现——见下。⚠️ 这条已经发生过一次：规则改了，本文六处描述同时作废，靠 `grep 头 20 个键` 才找回来** |
...
- **注意**：闸对原始文本做正则时，本文件里写出的符号名（`copy_to_clipboard`、`VK_OEM_PLUS`…）
  会被当成代码扫到。闸要么排除 `dev-docs/`，要么把本文件当成表的一侧而不是代码的一侧。
...
> ⚠️ **第 6 条（`Shift+Home` / `Shift+PageUp`）要先看包**，见 §2.1 的判包步骤。
> - **旧包**：`[key]` 行只记带 Ctrl/Alt/Win 的键，所以**第 6 条根本不会有这一行**
>   ——它要用别的读数，见表内。
> - **新包**：每一种键第一次按下都出声，所以第 6 条**和第 1–5 条一样可判**，
>   ⚠️ **但那一次必须是这次会话里的第一次**（第二次以后安静是正常的）。
>
> （这一格是 6.1 那条的现场：写下 2.1 那条日志规则的同一天，本文里已经有两条判据
> 在等一个不会出现的读数。**这是扫出来的第二条**，第一条是 C-c1。
> ⚠️ **修的是那条规则，不是这两条判据**——判据本来就没错，是它们等的读数按设计
> 不会出现。）
