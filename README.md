...
| Known missing | **Some keybinding actions**: of the core's 72 the host implements 60, refuses 7 by name, and owes 5 (2026-09-08; implemented was 46). A refusal is an arm that says this platform has no such thing -- the GTK inspector, a tab overview -- rather than a tag number in a log. The 5 owed are secure input, the on-screen keyboard, toggle-visibility, background opacity and the quit timer. |
...
screen — **and "never finished loading" and "failed immediately" send you to
two different halves of the program to look.**
