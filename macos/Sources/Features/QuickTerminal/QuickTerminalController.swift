...
class QuickTerminalController: BaseTerminalController {
    ...
    override func confirmCloseAsync(messageText: String, informativeText: String, confirmButtonTitle: String = String(localized: "Close", comment: "关闭确认框")) async -> NSApplication.ModalResponse? {
    ...
    private func showNoNewTabAlert() {
        guard let window else { return }
        let alert = NSAlert()
        alert.messageText = String(localized: "Cannot Create New Tab", comment: "快捷终端的提醒框")
        alert.informativeText = String(localized: "Tabs aren't supported in the Quick Terminal.", comment: "快捷终端的提醒框")
        alert.addButton(withTitle: String(localized: "OK", comment: "快捷终端的提醒框"))
