...
pub fn init(
    ...
    // be scoped here to be valid.
    {
        ...
        // but an MCP client only loads servers it was configured with.
        app.ensureMcpRegistered(
            config.@"poltergeist-mcp" and config.@"poltergeist-register-mcp",
        );
...
fn searchCallback_(
    ...
    switch (event) {
        .viewport_matches => |matches_unowned| {
            ...
            // ⚠️ **This message carries an arena the renderer takes over
            // and frees.** A delivery that fails hands ownership back here,
            // and dropping it on the floor would be a leak -- so the bounded
            // send has to be paired with a release at every site whose
            // message owns something. Bounded is not free; it is free only
            // for messages that are pure values.
            const vp_msg: rendererpkg.Message = .{ .search_viewport_matches = .{
                .arena = arena,
                .matches = matches,
            } };
            if (!rendererpkg.Thread.send(self.renderer_thread.mailbox, rendererpkg.Thread.wakerFor(&self.renderer_thread.wakeup), vp_msg)) {
                var undelivered = vp_msg.search_viewport_matches;
                undelivered.arena.deinit();
            }
        ...
        .selected_match => |selected_| {
            if (selected_) |sel| {
                ...
                // Owns an arena, same as above.
                const sel_msg: rendererpkg.Message = .{ .search_selected_match = .{
                    .arena = arena,
                    .match = match,
                } };
                if (!rendererpkg.Thread.send(self.renderer_thread.mailbox, rendererpkg.Thread.wakerFor(&self.renderer_thread.wakeup), sel_msg)) {
                    if (sel_msg.search_selected_match) |m| {
                        var undelivered = m;
                        undelivered.arena.deinit();
                    }
                }
        ...
        .quit => {
            ...
            // Owns an arena, same as above.
            const clear_msg: rendererpkg.Message = .{ .search_viewport_matches = .{
                .arena = .init(self.alloc),
                .matches = &.{},
            } };
            if (!rendererpkg.Thread.send(self.renderer_thread.mailbox, rendererpkg.Thread.wakerFor(&self.renderer_thread.wakeup), clear_msg)) {
                var undelivered = clear_msg.search_viewport_matches;
                undelivered.arena.deinit();
            }
            try self.renderer_thread.wakeup.notify();
...
fn setSelectionAndCopy(self: *Surface, sel: terminal.Selection) !void {
    ...
    const targets = copyOnSelectTargets(self.config.copy_on_select);
    if (targets.len == 0) return;
    try self.copySelectionToClipboards(sel, targets, .mixed);
}

/// Which clipboards `copy-on-select` writes. Nothing here checks whether
/// the host has a selection clipboard: `.selection` is handed to the apprt's
/// `setClipboard` regardless, and a host without one (Windows) refuses it
/// there, so `.primary` copies nothing on such a host.
///
/// ⚠️ **`.clipboard` kept its name across 1.4 and changed its meaning**: it
/// used to write both clipboards (that is `.both` now) and it now writes the
/// standard one only. A rename is caught by the compiler; this is not, which
/// is why it is a function with a test rather than an inline switch.
fn copyOnSelectTargets(mode: configpkg.CopyOnSelect) []const apprt.Clipboard {
    return switch (mode) {
        .none => &.{},
        .primary => &.{.selection},
        .clipboard => &.{.standard},
        .both => &.{ .standard, .selection },
    };
}

test "copy-on-select clipboard writes the standard clipboard and not the selection" {
    const testing = std.testing;
    const got = copyOnSelectTargets(.clipboard);
    try testing.expectEqual(@as(usize, 1), got.len);
    try testing.expectEqual(apprt.Clipboard.standard, got[0]);
    for (got) |c| try testing.expect(c != .selection);

    // The neighbours, so a swap between arms is caught too.
    try testing.expectEqual(@as(usize, 0), copyOnSelectTargets(.none).len);
    try testing.expectEqualSlices(apprt.Clipboard, &.{.selection}, copyOnSelectTargets(.primary));
    try testing.expectEqualSlices(apprt.Clipboard, &.{ .standard, .selection }, copyOnSelectTargets(.both));
}

/// The clipboard a middle click pastes from, or null for "do nothing".
/// `primary-paste` does nothing without a selection clipboard: upstream
/// dropped the old fallback to the standard clipboard, which is why the
/// Windows default is `clipboard-paste` (see `middle-click-action`).
fn middleClickSource(
    action: configpkg.MiddleClickAction,
    supports_selection: bool,
) ?apprt.Clipboard {
    return switch (action) {
        .ignore => null,
        .@"clipboard-paste" => .standard,
        .@"primary-paste" => if (supports_selection) .selection else null,
    };
}

test "a middle click on Windows pastes something by default" {
    // The Windows host declares `supports_selection_clipboard: false`
    // (windows/host/src/main.rs). With upstream's default this is null: a
    // middle click that does nothing, which no other test would notice.
    const got = middleClickSource(configpkg.Config.middleClickActionDefault(.windows), false);
    try std.testing.expectEqual(@as(?apprt.Clipboard, .standard), got);
}

test "middle-click primary-paste still reads the selection where there is one" {
    const testing = std.testing;
    try testing.expectEqual(
        @as(?apprt.Clipboard, .selection),
        middleClickSource(configpkg.Config.middleClickActionDefault(.linux), true),
    );
    try testing.expectEqual(@as(?apprt.Clipboard, .selection), middleClickSource(.@"primary-paste", true));
    try testing.expectEqual(@as(?apprt.Clipboard, null), middleClickSource(.@"primary-paste", false));
    try testing.expectEqual(@as(?apprt.Clipboard, null), middleClickSource(.ignore, true));
}
...
fn resize(self: *Surface, size: rendererpkg.ScreenSize) !void {
    ...
    //
    // ⚠️ Not upstream's bare `mailbox.push(..., .forever)`: every renderer
    // post in this file goes through `Thread.send` with `wakerFor`, because
    // on Windows a by-value copy of the `xev.Async` has no waiter and a
    // blocking push on a full mailbox never gets its consumer woken.
    _ = rendererpkg.Thread.send(
        self.renderer_thread.mailbox,
        rendererpkg.Thread.wakerFor(&self.renderer_thread.wakeup),
        .{ .resize = self.size },
