...
fn testStreamHandler(alloc: Allocator, dir: []const u8, token: []const u8) StreamHandler {
    return .{
        ...
        // Upstream added this field with no default, so leaving it out is a
        // compile error rather than a silent zero -- which is how the 0.8
        // merge found this fixture at all. `captureCommand` never reads it
        // (its one reader is the clipboard write path's `.max_size`), so it
        // gets the same `undefined` as the other fields this test does not
        // touch, rather than a real byte count that would suggest the test
        // says something about clipboard limits.
        .clipboard_write_limit = undefined,
        .history_token = token,
        .history_dir = dir,
