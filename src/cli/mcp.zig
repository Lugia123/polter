...
const tools = [_]Tool{
    .{
        .name = "me",
        .description = "Which terminal this agent is running in, and whether it is supervising or supervised. When it is watched, `sampling` says whether anything is measuring it, with the same three values terminal_list documents: `starting` (not confirmed yet, not a fault), `running`, `failed`. `call_silent_ms` is how long since this terminal last called a Polter tool, as terminal_list documents it. If you are a supervisor and it passes `poltergeist-calls-silent-after`, your tab's flag goes hollow (⚑ → ⚐) so the person can see it -- your own notices are the one place that could not tell anyone you had stopped.",
        .schema =
    ...
    .{
        .name = "terminal_list",
        .description = "Every terminal Polter knows about, and enough about each to tell them apart: `cwd` and `title` say which terminal this actually is, `role` and `shielded` say whether a call of yours will reach it, `held` says the user is holding it to its work. For a terminal somebody is watching there is also `quiet_ms` and `rounds` -- how long its screen has been unchanged, and how many times that has been reported. And `sampling`, which says whether anything is actually measuring under the watch -- `watching` is only the mark: **`starting`** means asked for and not yet confirmed. It is what every watch reads for the moment after it is made, and a running sampler confirms it within a few seconds even when its answer was lost; **it is not a fault, so do not close or restart a terminal for reading `starting`**. Still `starting` well after that means nothing has confirmed it -- the answer that it failed may itself have been lost -- so look at it with terminal_read before waiting on it any longer. **`running`** means the sampler has confirmed, or has sent something only a running sampler sends. **`failed`** means the terminal said it could not sample -- typically its command never started -- so nothing is measuring it and no quiet notice will come from it: look at it with terminal_read rather than waiting. **`quiet_ms` is absent rather than zero for a terminal nobody is watching**: nothing samples it, and zero would read as busy this instant. `call_silent_ms` is the other clock: how long since that terminal's agent last called a Polter tool. **Absent means it never has** -- a plain shell, an agent that has not started -- which is not the same as a long time. A large `call_silent_ms` with a small `quiet_ms` is a screen that is moving while nothing is being asked of Polter: a retry loop looks like that, and so does a worker waiting on a permission prompt or a long build. It is a reason to terminal_read, not a finding. The sidecar's own background traffic is not counted, and neither is persona_face. `window` and `tab` say **where** it is: two terminals are in the same window when their `window` values are equal, and in the same tab when their `tab` values are. That is the only thing those numbers mean -- they are not handles, nothing can be looked up by them, and they change every restart. Use them instead of guessing from `cwd` and `title` which terminals the user split out of one tab. ⚠️ **Absent means nobody said, not \"they are apart\"**: the interface has to be asked this question and not every one answers it -- on Linux none of them do, so both are absent for every terminal there. Reading absence as separate windows invents an arrangement. Durations and bookkeeping only -- call terminal_read to see what is on a screen.",
        .schema =
        \\{"type":"object","properties":{},"additionalProperties":false}
        ,
    },
    .{
        .name = "notices",
        .description = "What has happened that you have not been shown yet: which terminals went quiet and for how long, and which came back to work. **A second kind of clause, `0x… no tool call 22m (screen changed 3s ago)`, is a different clock**: not the screen, but how long since that terminal's agent last called a Polter tool. It exists because a screen can keep changing after the work has stopped -- a CLI stuck retrying a network error redraws its countdown for ever and never goes quiet -- so a terminal can be absent from the quiet clauses and still be here. It comes at `poltergeist-calls-silent-after` (15 minutes unless the user changed it) and again each further time that much passes, and only for a terminal that has called at least once. ⚠️ **It is arithmetic, not a verdict: it does not mean the terminal is dead.** It may be stuck in a loop; it may be waiting on a permission prompt a person has to answer; it may be running a long build. What to do with it is terminal_read, and decide from what is on the screen -- **not close it, restart it or interrupt it on this line alone.** The bracket is the screen's own figure, absent when nothing samples it. The same line can carry one clause about a group you supervise -- a task nothing has happened to for a long time, a conversation that has stopped, a task handed round three times -- each with its duration and no verdict attached, because none of those is by itself a problem. Reading clears them, so what comes back will not come back again. You are also handed this on a timer; call it yourself whenever you finish something, rather than waiting to be interrupted. An empty answer means nothing is waiting. Supervisor only.",
        .schema =
        \\{"type":"object","properties":{},"additionalProperties":false}
        ,
    },
