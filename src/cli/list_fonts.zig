...
fn runArgs(alloc_gpa: Allocator, argsIter: anytype) !u8 {
    ...
    if (comptime font.Discover == void) {
        ...
        var stderr_writer = std.Io.File.stderr().writerStreaming(global.io(), &buffer);
        const stderr = &stderr_writer.interface;
        try stderr.print(
            \\Polter was built without a font discovery mechanism. This is a compile-time
            \\option. Please review how Polter was built from source, contact the
