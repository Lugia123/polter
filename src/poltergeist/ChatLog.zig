...
// The record's shape -- how a name becomes a directory, what a day is, and
...
const day_bytes = daylog.day_bytes;
const max_segment = daylog.max_segment;
...
pub fn open(alloc: Allocator, io: std.Io, state_dir: []const u8) Error!ChatLog {
    ...
    var self: ChatLog = .{
        ...
        .tree = .{
            ...
            .dir = dir,
            .label = "chat log",
            // Only the chat record numbers its lines, so only the chat
            // record can say how far the record already goes.
            .probe = tailSeq,
        },
    };
