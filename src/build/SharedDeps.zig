...
pub fn add(
    ...
    //
    // ⚠️ Not upstream's single list: **mingw has no `pwd.h`**, and unlike the
    // `@cImport`s this replaced (which were lazy, and `os/passwd.zig`'s was
    // `if (builtin.os.tag != .windows)`), a module import is translated on
    // every build of this step -- so the Windows libghostty build would fail
    // here whether or not anything reaches `getpwuid`. The module still
    // exists on Windows so that the top-level `@import("posix_c")`s resolve.
    ...
    // **The target has to be passed, unlike upstream.** `pkg/opengl`
    // translates the glad headers with translate-c, and a dependency given
    // no target is built for the host. On a Mac host `khrplatform.h` then
    // takes its non-`_WIN64` branch and `GLsizeiptr` becomes `c_long`, which
    // is 32 bits on the Windows target the module is actually compiled for,
    // so every `isize` passed as a buffer size stops compiling (#801). On
    // Win64 the OpenGL ABI makes it `long long` -- 64 bits -- and that is
    // only what you get when the headers are read for Windows. Upstream
    // never notices: they build OpenGL on Linux, where host and target agree.
